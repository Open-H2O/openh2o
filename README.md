...
OpenH2O helps a water agency manage its own water data — the measurements, deliveries, wells, diversions, recharge, and drinking-water quality records that make up an agency's files — so the agency owns and understands its own basin. Satellite evapotranspiration (ET) is one of those data feeds, not the centerpiece. For agencies that also file with the state, it works out the numbers those filings need and lays them out to the GEARS and CalWATRS specifications — but **it cannot file them, and neither state system can currently accept a file it produces.** Reporting is an optional add-on, not the reason the platform exists. It is built on a fully open stack so that any agency — or any engineering firm working on their behalf — can run it, read it, and improve it.
...
- **GEARS / CalWATRS** — the State Water Resources Control Board's reporting systems. **OpenH2O does not file to either one, and neither can currently accept a file it produces.** Both are large state platforms that would need work on their own side before they could take data from a system like this one, and that work has not been done. What OpenH2O does is calculate the figures a filing needs and write them out in those systems' own layouts, so an agency can check its own numbers and keep its own copy. Getting them into the state's hands is the agency's own job, by whatever route the Board gives them, and the person who certifies a filing does so under penalty of perjury. Direct submission from OpenH2O is not available and should not be counted on.
...
- **It runs the routine work.** Monthly data sync, pruning dead monitoring stations, working out the figures a GEARS or CalWATRS filing would need, reading the health check — the agent can do these on a schedule and explain what each number means. It cannot file anything with the state, and neither can the platform.
...
- **State report figures** — the numbers a GEARS filing needs (by-well and by-ET) and a CalWATRS one (direct-use and to-storage), calculated and written out as CSV in each system's own layout. ⚠ **This is not a filing path.** Neither GEARS nor CalWATRS can currently accept a file from this platform; both would need work on the state's side first. Use these to check your own figures, not as something you can submit.
