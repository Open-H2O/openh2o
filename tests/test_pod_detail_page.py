...
class TestPodRecordsGroupedByWaterYear:
    ...
    def test_prior_year_gets_its_own_group_and_subtotal(self):
        html = _pod_page(self._two_year_pod())
        # colspan is 7, not 6: 146-02 Task 3 (ISS-181) added an Actions column
        # (Edit / Delete per row), and 146-03 Task 2 added "How known" beside
        # it (method, device nickname, data state).
        assert re.search(
            r'<tr class="row-group">\s*<td colspan="7">' + re.escape(PRIOR_NAME), html
        )
        subtotals = _row_subtotal_blocks(html)
