...
the panel's six totals, FIG-accounting-036..040, six sites that repeat six
...
`FIG-accounting-028..041`) moved out of `_dashboard_content.html` into their own
...
`FIG-accounting-036..040`, each the same context value as a panel figure and so
...
still match to the cent: `FIG-accounting-034` 2,901.42 to **957.22** and
`FIG-accounting-035` 2,901.42 to **957.22**, both because the pinned account's
...
the new basis: `FIG-accounting-034` 19,101.42 to **2,901.42** (the 16,200.00 AF
of surface entitlement it used to add is no longer counted as groundwater),
`FIG-accounting-035` 14,511.81 to **2,901.42** (this account pumps nothing), and
`FIG-accounting-059` −136.03 to **911.58** (750.32 + 303.40 carry-over − 142.14
...
| `FIG-accounting-050` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:93` | Balance | `grand_net` | `accounting/views.py:248` | `— (arithmetic in the view)` | `parcels_parcelledger, accounting_calculationrun` | 1038.11 | 1038.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1249.07): same context variable as `FIG-accounting-041`; consumptive use did not move, so the whole shift is Supplies (`FIG-accounting-045`) minus consumptive use. Independence: different identity. Supplies minus consumptive use, both above. |
| `FIG-accounting-046` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:53` | Supplies by source › Surface | `grand_supply_surface` | `accounting/views.py:195` | `account_consumptive_balance` | `parcels_parcelledger` | 11115.67 | 11115.67 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 11407.71): same context variable as `FIG-accounting-036`; the golden's Stevinson headgate re-cap (148-02 Task 5) caps its seven center-pivot fields at demand ÷ 0.87 instead of demand ÷ 0.75, a lower raw delivery for the same demand, so the district-wide delivered magnitude fell. Independence: different identity. Magnitude of the delivery rows, which are stored as negative numbers by the platform's own convention. |
| `FIG-accounting-047` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:54` | Supplies by source › Groundwater | `grand_supply_groundwater` | `accounting/views.py:196` | `account_consumptive_balance` | `parcels_parcelledger` | 4458.45 | 4458.45 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 4377.38): same context variable as `FIG-accounting-037`; Task 1's `subtract_surface_water` now subtracts only a canal delivery's consumed share (delivered × efficiency) from a parcel's ET demand, not the whole delivered magnitude, so well parcels district-wide that used to be fully covered now carry a small calculated-groundwater estimate (measured on `MER-APN-032`, this account's own pinned parcel, below). Independence: different identity. Absolute value OF THE SUM of the negative non-delivery rows, not the sum of absolute values. Transcribed as written; the two differ if a positive row ever lands in that set. |
...
| `FIG-accounting-030` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:75` | Supplies › Rain | `row.precip` | `accounting/views.py:187` | `account_consumptive_balance` | `accounting_calculationrun` | 420.12 | 420.12 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-031` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:78` | Supplies › Total | `row.supply_total` | `accounting/views.py:188` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 5926.78 | 5926.78 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 5921.09): follows `FIG-accounting-029` (Groundwater 0.00 → 5.69); Surface and Rain did not move. Independence: restatement. The three supply columns added. |
| `FIG-accounting-033` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:84` | Balance | `row.net_vs_supply` | `accounting/views.py:189` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1337.17 | 1337.17 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1331.48): Supplies moved (`FIG-accounting-031`) and Consumptive use did not, so the whole shift is the Supplies move. Independence: restatement. |
...
| `FIG-accounting-035` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:91` | Groundwater budget › Remaining | `row.remaining` | `accounting/views.py:200` | `— (arithmetic in the view)`, `charged_groundwater` | `accounting_allocationplan, accounting_watertype, geography_parcelzone, parcels_parcelledger, accounting_calculationrun` | 950.11 | 950.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 Task 4 (was 957.22): the MEANING changed, not just the data. Remaining now subtracts `charged_groundwater`, not the Groundwater column's consumed figure: this account's one `calculated` row (`MER-APN-032`, `FIG-accounting-029`) is billed at its `CalculationRun.gw_extracted_af`, the consumed 5.6912 AF divided by the run's stamped groundwater efficiency (0.80), i.e. 7.11 AF extracted rather than 5.69 AF consumed, so Remaining is 957.22 − 7.11 = 950.11. Recomputed by joining the account's `calculated` ledger row to its `CalculationRun` on (parcel, period_start = effective_date) and reading the stamped extraction figure, transcribing `accounting/services.py:charged_groundwater`. Re-measured 2026-09-06 after 136-02 (was 2,901.42): the re-sized allocation above, and this account still pumps 0.00 (`FIG-accounting-029`), so Remaining moved with it and nothing else changed. Re-measured 2026-09-06 after 136-01 (was 14,511.81): groundwater allocation minus groundwater use, and this account pumps 0.00 (`FIG-accounting-029`), so it equals the allocation. Before 136-01 the column subtracted gross evapotranspiration (ISS-151). Independence: restatement. |
| `FIG-accounting-036` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:107` | All 11 active accounts › Surface | `grand_supply_surface` | `accounting/views.py:227` | `account_consumptive_balance` | `parcels_parcelledger` | 11115.67 | 11115.67 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 11407.71): same context variable as `FIG-accounting-046`, so that row's reason is this row's reason (the Stevinson headgate re-cap). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-046` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-037` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:108` | All 11 active accounts › Groundwater | `grand_supply_groundwater` | `accounting/views.py:228` | `account_consumptive_balance` | `parcels_parcelledger` | 4458.45 | 4458.45 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 4377.38): same context variable as `FIG-accounting-047`, so that row's reason is this row's reason (Task 1's consumed-only canal subtraction). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-047` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
...
| `FIG-accounting-039` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:110` | All 11 active accounts › Total | `grand_supply_total` | `accounting/views.py:226` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 17247.39 | 17247.39 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 17458.35): same context variable as `FIG-accounting-045`; Surface fell and Groundwater rose (`FIG-accounting-036`, `FIG-accounting-037`), Rain flat. New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-045` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
...
| `FIG-accounting-041` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:112` | All 11 active accounts › Balance | `grand_net` | `accounting/views.py:283` | `— (arithmetic in the view)` | `parcels_parcelledger, accounting_calculationrun` | 1038.11 | 1038.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1249.07): same context variable as `FIG-accounting-050`; consumptive use did not move so the whole shift is the Supplies total's move (`FIG-accounting-039`). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-050` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-055` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:269` | Consumptive use | `row.consumptive_use_gross` | `accounting/views.py:235` | `zone_consumptive_balance` | `accounting_calculationrun` | 1189.75 | 1189.75 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-051` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:257` | Supplies › Surface | `row.surface` | `accounting/views.py:237` | `zone_consumptive_balance` | `parcels_parcelledger` | 1170.93 | 1170.93 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-052` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:260` | Supplies › Groundwater | `row.groundwater` | `accounting/views.py:238` | `zone_consumptive_balance` | `parcels_parcelledger` | 142.14 | 142.14 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-053` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:263` | Supplies › Rain | `row.precip` | `accounting/views.py:239` | `zone_consumptive_balance` | `accounting_calculationrun` | 110.56 | 110.56 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-054` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:266` | Supplies › Total | `row.supply_total` | `accounting/views.py:240` | `zone_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1423.63 | 1423.63 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-056` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:272` | Balance | `row.net_vs_supply` | `accounting/views.py:241` | `zone_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 233.88 | 233.88 | MATCH | MATCH | Independence: restatement. |
...
`FIG-accounting-043` and `FIG-accounting-067`, and both are allocation totals;
...
fourth row, `FIG-accounting-043`, carries `MATCH · ISS-156` and still does).
...
`FIG-accounting-055`, `048` and `049` each carried this on 2026-09-05. The
...
The two surviving figures are `FIG-accounting-055` and `048`.
...
- **`FIG-accounting-060`** (`053` until 136-01)**.** The methodology preview's fallback line, shown when
...
| `FIG-geography-005` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:132` | Allocation vs. use panel, Remaining (the page's one `.budget-seg--result`) | `b.remaining` | `geography/views.py:285`, computed via `zone_groundwater_budget` at `accounting/services.py:1246` | `zone_groundwater_budget` calls `available_with_carryover` | `accounting_allocationplan, accounting_watertype, accounting_allocationcarryover, parcels_parcelledger, geography_parcelzone` | 911.58 | 911.58 | MATCH | MATCH | New site, 143-06 Task 4: repeats the table's Remaining cell (`FIG-geography-009`) and the dashboard's zone row (`FIG-accounting-059`) as the page's ONE lead figure, rule 8, the checkpoint's zone-lead ruling. 1,053.72 (`FIG-geography-001`) minus 142.14 (`FIG-geography-004`) is 911.58 by hand. Independence: restatement, verified a second way by hand subtraction. |
