...
`FIG-recharge-003` (the panel) and `FIG-recharge-008` (the table's own
...
`FIG-recharge-005` is the same breakdown segment's row, not reachable on THIS
...
| `FIG-wells-005` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:313` | Close | `row.close` | `wells/views.py:164` | `water_level_history` (`wells/measurement_history.py:103-126`) | `measurements_watermeasurement` | 101.92 | 101.92 | MATCH | MATCH | Was `FIG-wells-003` at line 195. 143-10 Task 4 dropped the "(ft)" suffix from this column's own header (rule 4: the unit now states once, in the digest's own facts line above the table) and moved this table beside the meter table in a `page-grid-2col` row instead of stacking under it; no stored column holds a month's close, the view builds it in Python as the last reading in the month, taken in America/Los_Angeles; the value did not move. Independence: restatement, the check re-derives the same selection rule rather than reading a number back. |
| `FIG-wells-006` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:314` | Change | `row.change` | `wells/views.py:164` | `water_level_history` (`wells/measurement_history.py:128-132`) | `measurements_watermeasurement` | 3.13 | 3.13 | MATCH | MATCH | Was `FIG-wells-004` at line 196. Same header change as the row above: September 2026's close (101.9163) minus August 2026's close (98.7905); the value did not move. Independence: restatement, same reasoning as FIG-wells-005. A second well, MER-W-004 (well id 13, `/wells/13/`, captured as evidence rather than a figure site), carries the two-year dry-year decline ISS-145 names: its logger's close moves from 94.15 ft in September 2025 to 109.65 ft in September 2026, a 15.50 ft rise. |
...
`FIG-recharge-005`, the "by water type" breakdown, does not render on a site
