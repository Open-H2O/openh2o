...
**This ledger is complete.** All 123 figures the platform renders have a row, and
...
The platform renders 123 figures across 24 page templates (120 until 2026-09-24, when 148-02 gave the use-area balance panel a Deep percolation row, FIG-parcels-007, with the parcel ids after it moving up by one, and gave the calculation page its extracted figure and the divisor behind it, FIG-accounting-009 and -010, and every accounting id after them moved up by two; 114 until 2026-09-08,
when 143-01 gave the account balance panel's Use area table a footer carrying
the panel's six totals, FIG-accounting-023..026, six sites that repeat six
...
the panel's six totals, FIG-accounting-038..040, six sites that repeat six
...
**Re-numbered 2026-09-16 after 143-13 extracted the Active water accounts table.**
The table's fourteen sites (the eight per-row cells and the six footer cells,
`FIG-accounting-030..041`) moved out of `_dashboard_content.html` into their own
partial, `templates/accounting/partials/_active_accounts_table.html`, shared by
`{% include %}` with the new accounts list page (`/accounting/accounts/`, Brent's
"accounts-table" checkpoint ruling: the list becomes this same table, with a
search), so both screens share one arithmetic path and one set of figure sites
rather than a second copy the two could drift apart from. No figure was added, removed,
or recomputed differently; every accounting id from 028 on renumbered the same way
the two prior re-numberings below did, and the 2026-09-06 measurements stand.
...
`FIG-accounting-038..040`, each the same context value as a panel figure and so
...
still match to the cent: `FIG-accounting-036` 2,901.42 to **957.22** and
`FIG-accounting-037` 2,901.42 to **957.22**, both because the pinned account's
...
the new basis: `FIG-accounting-036` 19,101.42 to **2,901.42** (the 16,200.00 AF
of surface entitlement it used to add is no longer counted as groundwater),
`FIG-accounting-037` 14,511.81 to **2,901.42** (this account pumps nothing), and
`FIG-accounting-061` −136.03 to **911.58** (750.32 + 303.40 carry-over − 142.14
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `FIG-accounting-047` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:43` | Supplies | `grand_supply_total` | `accounting/views.py:194` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun, accounting_wateraccountparcel` | 17247.39 | 17247.39 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 17458.35): same context variable as `FIG-accounting-041`, the Active water accounts table's footer; the Stevinson headgate re-cap (lower surface deliveries) and Task 1's consumed-only canal subtraction (higher groundwater use on well parcels) moved it. Independence: different identity. Summed over active accounts only. Also computed in ONE pass over the distinct union of those parcels, a second identity that would disagree if any parcel belonged to two active accounts. It does not: none does. |
| `FIG-accounting-051` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:71` | Consumptive use | `grand_consumptive_use` | `accounting/views.py:193` | `account_consumptive_balance` | `accounting_calculationrun` | 16209.28 | 16209.28 | MATCH | MATCH | Independence: different identity. Same single-pass cross-check, same agreement. |
| `FIG-accounting-052` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:93` | Balance | `grand_net` | `accounting/views.py:248` | `— (arithmetic in the view)` | `parcels_parcelledger, accounting_calculationrun` | 1038.11 | 1038.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1249.07): same context variable as `FIG-accounting-043`; consumptive use did not move, so the whole shift is Supplies (`FIG-accounting-047`) minus consumptive use. Independence: different identity. Supplies minus consumptive use, both above. |
| `FIG-accounting-048` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:53` | Supplies by source › Surface | `grand_supply_surface` | `accounting/views.py:195` | `account_consumptive_balance` | `parcels_parcelledger` | 11115.67 | 11115.67 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 11407.71): same context variable as `FIG-accounting-038`; the golden's Stevinson headgate re-cap (148-02 Task 5) caps its seven center-pivot fields at demand ÷ 0.87 instead of demand ÷ 0.75, a lower raw delivery for the same demand, so the district-wide delivered magnitude fell. Independence: different identity. Magnitude of the delivery rows, which are stored as negative numbers by the platform's own convention. |
| `FIG-accounting-049` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:54` | Supplies by source › Groundwater | `grand_supply_groundwater` | `accounting/views.py:196` | `account_consumptive_balance` | `parcels_parcelledger` | 4458.45 | 4458.45 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 4377.38): same context variable as `FIG-accounting-039`; Task 1's `subtract_surface_water` now subtracts only a canal delivery's consumed share (delivered × efficiency) from a parcel's ET demand, not the whole delivered magnitude, so well parcels district-wide that used to be fully covered now carry a small calculated-groundwater estimate (measured on `MER-APN-032`, this account's own pinned parcel, below). Independence: different identity. Absolute value OF THE SUM of the negative non-delivery rows, not the sum of absolute values. Transcribed as written; the two differ if a positive row ever lands in that set. |
| `FIG-accounting-050` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:55` | Supplies by source › Rain | `grand_supply_precip` | `accounting/views.py:197` | `account_consumptive_balance` | `accounting_calculationrun` | 1673.27 | 1673.27 | MATCH | MATCH | Independence: different identity. Effective rainfall, from the calculation runs rather than the ledger. |
| `FIG-accounting-034` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:81` | Consumptive use | `row.consumptive_use_gross` | `accounting/views.py:183` | `account_consumptive_balance` | `accounting_calculationrun` | 4589.61 | 4589.61 | MATCH | MATCH | Independence: restatement. Sum of gross evapotranspiration over the account's parcels' monthly runs. |
| `FIG-accounting-030` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:69` | Supplies › Surface | `row.surface` | `accounting/views.py:185` | `account_consumptive_balance` | `parcels_parcelledger` | 5500.97 | 5500.97 | MATCH | MATCH | Independence: restatement. Same delivery-magnitude rule as FIG-accounting-048. |
| `FIG-accounting-031` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:72` | Supplies › Groundwater | `row.groundwater` | `accounting/views.py:186` | `account_consumptive_balance` | `parcels_parcelledger` | 5.69 | 5.69 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 0.00): no longer a real zero. `MER-APN-032`, this account's well parcel, now carries a `calculated` row every month (0.4825 AF down to 0.0001 AF, summing to 5.6912): Task 1's `subtract_surface_water` subtracts only the canal delivery's consumed share from ET, not the whole delivered magnitude as before, so a small residual survives where the old full-delivery subtraction used to cover it exactly. Independence: restatement. |
| `FIG-accounting-032` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:75` | Supplies › Rain | `row.precip` | `accounting/views.py:187` | `account_consumptive_balance` | `accounting_calculationrun` | 420.12 | 420.12 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-033` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:78` | Supplies › Total | `row.supply_total` | `accounting/views.py:188` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 5926.78 | 5926.78 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 5921.09): follows `FIG-accounting-031` (Groundwater 0.00 → 5.69); Surface and Rain did not move. Independence: restatement. The three supply columns added. |
| `FIG-accounting-035` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:84` | Balance | `row.net_vs_supply` | `accounting/views.py:189` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1337.17 | 1337.17 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1331.48): Supplies moved (`FIG-accounting-033`) and Consumptive use did not, so the whole shift is the Supplies move. Independence: restatement. |
| `FIG-accounting-036` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:88` | Groundwater budget › Allocation | `row.allocation` | `accounting/views.py:181` | `— (arithmetic in the view)` | `accounting_allocationplan, accounting_watertype, geography_parcelzone, accounting_wateraccountparcel` | 957.22 | 957.22 | MATCH | MATCH | Re-measured 2026-09-06 after 136-02 (was 2,901.42): the two groundwater plans this account's share is pro-rated from were re-sized to their own demonstration sustainable-yield rates, and Halvern Valley GSA's fell from 9,689.40 to 2,809.93 at 0.58 AF/acre. Thirteen of that district's 46 parcel-zone rows and five of Halvern Irrigation-Urban GSA's 23 belong to this account, so 13/46 of 2,809.93 plus 5/23 of 750.32 gives 957.22. Re-measured 2026-09-06 after 136-01 (was 19,101.42): groundwater plans only, so the 16,200.00 AF surface entitlement this account also holds is no longer added in (like with like; DESIGN.md rule 12). Independence: restatement. Pro-rated by how many of the account's parcel-zone rows fall in each zone. The only figure on this screen computed in the view itself, and the reconciliation had to transcribe that division to reproduce it. |
| `FIG-accounting-037` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:91` | Groundwater budget › Remaining | `row.remaining` | `accounting/views.py:200` | `— (arithmetic in the view)`, `charged_groundwater` | `accounting_allocationplan, accounting_watertype, geography_parcelzone, parcels_parcelledger, accounting_calculationrun` | 950.11 | 950.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 Task 4 (was 957.22): the MEANING changed, not just the data. Remaining now subtracts `charged_groundwater`, not the Groundwater column's consumed figure: this account's one `calculated` row (`MER-APN-032`, `FIG-accounting-031`) is billed at its `CalculationRun.gw_extracted_af`, the consumed 5.6912 AF divided by the run's stamped groundwater efficiency (0.80), i.e. 7.11 AF extracted rather than 5.69 AF consumed, so Remaining is 957.22 − 7.11 = 950.11. Recomputed by joining the account's `calculated` ledger row to its `CalculationRun` on (parcel, period_start = effective_date) and reading the stamped extraction figure, transcribing `accounting/services.py:charged_groundwater`. Re-measured 2026-09-06 after 136-02 (was 2,901.42): the re-sized allocation above, and this account still pumps 0.00 (`FIG-accounting-031`), so Remaining moved with it and nothing else changed. Re-measured 2026-09-06 after 136-01 (was 14,511.81): groundwater allocation minus groundwater use, and this account pumps 0.00 (`FIG-accounting-031`), so it equals the allocation. Before 136-01 the column subtracted gross evapotranspiration (ISS-151). Independence: restatement. |
| `FIG-accounting-038` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:107` | All 11 active accounts › Surface | `grand_supply_surface` | `accounting/views.py:227` | `account_consumptive_balance` | `parcels_parcelledger` | 11115.67 | 11115.67 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 11407.71): same context variable as `FIG-accounting-048`, so that row's reason is this row's reason (the Stevinson headgate re-cap). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-048` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-039` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:108` | All 11 active accounts › Groundwater | `grand_supply_groundwater` | `accounting/views.py:228` | `account_consumptive_balance` | `parcels_parcelledger` | 4458.45 | 4458.45 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 4377.38): same context variable as `FIG-accounting-049`, so that row's reason is this row's reason (Task 1's consumed-only canal subtraction). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-049` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-040` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:109` | All 11 active accounts › Rain | `grand_supply_precip` | `accounting/views.py:229` | `account_consumptive_balance` | `accounting_calculationrun` | 1673.27 | 1673.27 | MATCH | MATCH | New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-050` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-041` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:110` | All 11 active accounts › Total | `grand_supply_total` | `accounting/views.py:226` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 17247.39 | 17247.39 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 17458.35): same context variable as `FIG-accounting-047`; Surface fell and Groundwater rose (`FIG-accounting-038`, `FIG-accounting-039`), Rain flat. New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-047` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-042` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:111` | All 11 active accounts › Consumptive use | `grand_consumptive_use` | `accounting/views.py:225` | `account_consumptive_balance` | `accounting_calculationrun` | 16209.28 | 16209.28 | MATCH | MATCH | New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-051` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-043` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:112` | All 11 active accounts › Balance | `grand_net` | `accounting/views.py:283` | `— (arithmetic in the view)` | `parcels_parcelledger, accounting_calculationrun` | 1038.11 | 1038.11 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 (was 1249.07): same context variable as `FIG-accounting-052`; consumptive use did not move so the whole shift is the Supplies total's move (`FIG-accounting-041`). New site, 142-01 (2026-09-08): the Active water accounts table's footer, added so the panel and the table are visibly one thing. It renders the SAME context value as `FIG-accounting-052` above, one variable and one number, so the recomputation at that row is the recomputation here; the rendered figure was read off the served footer on staging after the deploy. What makes the rows addable (DESIGN.md rule 12): one quantity, one period, every active account, and the footer's label says so. |
| `FIG-accounting-057` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:269` | Consumptive use | `row.consumptive_use_gross` | `accounting/views.py:235` | `zone_consumptive_balance` | `accounting_calculationrun` | 1189.75 | 1189.75 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-053` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:257` | Supplies › Surface | `row.surface` | `accounting/views.py:237` | `zone_consumptive_balance` | `parcels_parcelledger` | 1170.93 | 1170.93 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-054` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:260` | Supplies › Groundwater | `row.groundwater` | `accounting/views.py:238` | `zone_consumptive_balance` | `parcels_parcelledger` | 142.14 | 142.14 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-055` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:263` | Supplies › Rain | `row.precip` | `accounting/views.py:239` | `zone_consumptive_balance` | `accounting_calculationrun` | 110.56 | 110.56 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-056` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:266` | Supplies › Total | `row.supply_total` | `accounting/views.py:240` | `zone_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1423.63 | 1423.63 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-058` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:272` | Balance | `row.net_vs_supply` | `accounting/views.py:241` | `zone_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 233.88 | 233.88 | MATCH | MATCH | Independence: restatement. |
| `FIG-accounting-059` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:276` | Groundwater budget › Allocation | `row.allocation` | `accounting/views.py:247`, computed via `zone_groundwater_budget` at `:246` | `zone_groundwater_budget` | `accounting_allocationplan` | 750.32 | 750.32 | MATCH | MATCH | 136-02 left this where it was: Halvern Irrigation-Urban GSA's demonstration sustainable-yield rate is 2.00 AF/acre, which is the rate the plan already carried, so its allocation stayed 750.32 while the other two districts moved. Independence: restatement. Re-cited 2026-09-06 after 137-02 (ISS-154 closed): this cell now reads off the shared `zone_groundwater_budget` helper (`accounting/services.py:1134`), the same one `geography/views.py` calls for `FIG-geography-006`, instead of being computed twice. The value did not move. Re-cited 2026-09-12 after 143-06 Task 4 renumbered the district page's table figures to make room for its new lead panel (was `FIG-geography-001`). |
| `FIG-accounting-060` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:279` | Groundwater budget › Carried fwd | `row.carryover` | `accounting/views.py:269`, computed via `zone_groundwater_budget` at `:246` | `zone_groundwater_budget` calls `zone_carryover` | `accounting_allocationcarryover` | 79.80 | 79.80 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 Task 4 (was 303.40): the stored `AllocationCarryover` row itself moved, not just this reader. `rollover_allocations` rolls the closed WY2024-2025 forward through `water_year_usage_by_type`, whose GW bucket now adds the extraction difference (148-02 Task 4) on top of the recorded usage it already counted, so the closed year's groundwater use is charged higher and the signed surplus it carries into WY2025-2026 is smaller. Independence: restatement. 136-02 changed allocations only, not the recharge pool this column reads, so the carry-over did not move. Independence: restatement. Looked up by water year, which is named for the calendar year the year ENDS in. This zone's rows for that label sum to the figure shown. Re-cited 2026-09-06 after 137-02 (ISS-154 closed): reads off the same shared `zone_groundwater_budget` helper as `FIG-accounting-059`, and now equals the district page's own Carried forward cell for this zone and period (`FIG-geography-007`) to the cent. Re-cited 2026-09-12 after 143-06 Task 4's renumbering (was `FIG-geography-002`). |
| `FIG-accounting-061` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:282` | Groundwater budget › Remaining | `row.remaining` | `accounting/views.py:270`, computed via `zone_groundwater_budget` at `:246` | `zone_groundwater_budget` calls `available_with_carryover` and `charged_groundwater` | `accounting_allocationplan, accounting_watertype, accounting_allocationcarryover, parcels_parcelledger, accounting_calculationrun` | 676.93 | 676.93 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 Task 4 (was 911.58): both terms moved. Carried forward fell to 79.80 (`FIG-accounting-060`), and Remaining now subtracts `charged_groundwater` (the zone's `calculated` rows billed at their runs' stamped `gw_extracted_af`) rather than the Groundwater column's consumed figure (`FIG-accounting-054`, unchanged at 142.14): 750.32 + 79.80 − 153.19 extracted = 676.93. Independence: restatement. 136-02 moved none of this row's three terms, so it still reads 911.58. Re-measured 2026-09-06 after 136-01 (was −136.03): allocation 750.32 plus carry-over 303.40 (`FIG-accounting-060`) minus 142.14 of groundwater use (`FIG-accounting-054`). Re-cited 2026-09-06 after 137-02 (ISS-154 closed): this is the same `zone_groundwater_budget` call `FIG-geography-009` now reads, so the dashboard and the district page print the same 911.58 for this zone and period, to the cent, for the first time. Before ISS-154 was fixed the district page read this same zone at −562.75, because it counted canal water as pumping; without the carry-over the correct figure would have been 608.18. Independence: restatement. Re-cited 2026-09-12 after 143-06 Task 4's renumbering (was `FIG-geography-004`); the same 911.58 is now ALSO the district page's one lead figure, `FIG-geography-005`. |
...
| `FIG-parcels-007` | `/parcels/11/?period=2` | `templates/parcels/partials/_detail_pane.html:119` | Foot: Deep percolation | `mass_balance.deep_percolation_af` | `parcels/views.py:176` | `parcel_mass_balance` | `accounting_calculationrun` | 0.00 | 0.00 | MATCH | MATCH | Added 2026-09-24 by 148-02: the sum of the two deep-percolation outputs the engine now names (canal water delivered beyond what the crop could use, `surface_delivered_af - surface_water_af`, and pumped water beyond what the crop consumed, `deep_percolation_gw_af`), listed so the rows under Uses add up to Uses. Recomputed in SQL from those two columns over this field's WY 2025-2026 runs: 0.0000 + 0.0000 (a field with no canal delivery and no estimated pumping). Checked on a field where it is not zero, MER-APN-032 WY 2025-2026: 181.2471, and Uses 772.5241 minus consumptive use 591.2770 is 181.2471. Independence: different identity. The pane's other rows were not re-measured by 148-02 (its reconciliation was the dashboard's). |
...
| FIG-parcels-010 | `/parcels/11/?period=1` | Residual (segment) | -181.57 | -181.57 | MATCH | MATCH | |
...
| FIG-parcels-011 | `/parcels/11/?period=1` | Water use recorded, no supply reported | 181.57 | 181.57 | MATCH | MATCH | **Renders in the wet year too**, at a smaller figure. See finding 2 below: the checkpoint this plan was scoped against expected the line to disappear here, and it does not. |
...
| FIG-parcels-010 | `/parcels/32/?period=2` | Residual (segment) | 0.00 | 0.00 | MATCH | MATCH | Inside the 0.01 AF Balanced band. |
...
| FIG-parcels-010 | `/parcels/17/?period=2` | Residual (segment) | 132.90 | 132.90 | MATCH | MATCH | The same value the pane's two identities already agreed on before 137-01, because this parcel banks no recharge. |
...
| FIG-parcels-011 | `/parcels/17/?period=2` | Water use recorded, no supply reported | not rendered | 0.00 | NO VALUE | MATCH | Has a well; zero on the disposition the figure sums. The line correctly does not render (DESIGN.md rule 8: a zero prints nothing). |
...
UNVERIFIED figures (`FIG-parcels-012`, `-012`, the ET-not-computed branch) are
...
`FIG-accounting-045` and `FIG-accounting-069`, and both are allocation totals;
...
fourth row, `FIG-accounting-045`, carries `MATCH · ISS-156` and still does).
...
| `FIG-accounting-002` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:70` | Billable groundwater | `run.final_af` | `accounting/views.py:1232` | `— (model field, written by accounting/management/commands/run_calculations.py:272)` | `accounting_calculationrun` | 27.9945 | 27.9945 | MATCH | MATCH | Independence: different identity. Also checked against the magnitude of the `calculated` ledger row for the same parcel-month, which is what the page's own result card claims it equals (agrees to 0.0000), and against the last step of the waterfall, likewise 0.0000. Site moved from `:67` to `:70` when 143-09 named the run's fingerprint in the head card (R-045); the id did not move. |
| `FIG-accounting-003` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:103` | In (AF) | `step.input_af` | `accounting/views.py:1266` | `— (read out of the stored breakdown)` | `accounting_calculationrun` | 0.0000 | 0.0000 | MATCH | MATCH | Independence: restatement, plus a whole-table identity. Pinned to step 1, which starts at zero. Every one of the five steps was also checked for continuity, each step's In against the step above it's Out, and none breaks. Site moved from `:100` to `:103` (143-09, R-045); the id did not move. |
| `FIG-accounting-004` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:104` | Out (AF) | `step.output_af` | `accounting/views.py:1267` | `— (read out of the stored breakdown)` | `accounting_calculationrun, datasync_openetcache, parcels_parcel` | 39.8491 | 39.8491 | MATCH | MATCH | Independence: **different identity**. Rebuilt from the raw satellite reading, 81.119346 mm of evapotranspiration over 149.73 acres divided by 304.8, without reading the stored figure at all. The recomputation could have disagreed and did not. Site moved from `:101` to `:104` (143-09, R-045); the id did not move. |
| `FIG-accounting-005` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:118` | Deposited this month | `run.banked_af` | `accounting/views.py:1232` | `— (model field)` | `accounting_calculationrun` | *(not rendered)* | 0.0000 | NO VALUE | UNVERIFIED | The whole banked-water block sits behind a condition no row in this database satisfies: **0 of 1,824 calculation runs** carry a non-zero deposit or draw. Nothing was on screen to read. The stored value is 0.0000, and that is not the same claim. Site moved from `:115` to `:118` (143-09, R-045); the id did not move. |
| `FIG-accounting-006` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:122` | Drawn down this month | `run.drawn_af` | `accounting/views.py:1232` | `— (model field)` | `accounting_calculationrun` | *(not rendered)* | 0.0000 | NO VALUE | UNVERIFIED | Same block, same reason. Site moved from `:119` to `:122` (143-09, R-045); the id did not move. |
| `FIG-accounting-007` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:138` | Drawn (AF) | `draw.amount_af` | `accounting/views.py:1273` | `— (queryset)` | `accounting_watercreditdraw` | *(not rendered)* | *(no rows)* | NO VALUE | UNVERIFIED | Nested one level deeper still: the draws table needs a `WaterCreditDraw` row and **there are none**, though five water-credit rows are held. Site moved from `:135` to `:138` (143-09, R-045); the id did not move. |
| `FIG-accounting-008` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:156` | Final billable groundwater | `run.final_af` | `accounting/views.py:1232` | `— (model field)` | `accounting_calculationrun` | 27.9945 | 27.9945 | MATCH | MATCH | Independence: different identity, as `FIG-accounting-002`. The page states this figure twice, at the top and at the foot; both render the same value, which is a thing the page could get wrong and does not. Site moved from `:153` to `:156` (143-09, R-045); the id did not move. |
| `FIG-accounting-009` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:170` | Estimated groundwater extracted › the divisor | `gw_efficiency_applied` | `accounting/views.py:1916` | `— (arithmetic in the view: run.final_af / run.gw_extracted_af)` | `accounting_calculationrun` | 0.80 | 0.80 | MATCH | MATCH | Added 2026-09-24 by 148-02 (the page stated 0.88 as billable after Task 4 made the allocation spend extraction). Read back off the run's own two stamped figures, 27.9945 / 34.9931 = 0.8000, never off the live setting, so the page states what the charge was computed with. Independence: restatement. Absent on a metered, no-well or zero month. |
| `FIG-accounting-010` | `/accounting/calculation-run/16/2026-03/` | `templates/accounting/calculation_run_detail.html:174` | Estimated groundwater extracted | `run.gw_extracted_af` | `accounting/views.py:1908` | `— (model field, written by accounting/management/commands/run_calculations.py, final_af / SiteConfig.groundwater_efficiency at run time)` | `accounting_calculationrun, core_siteconfig` | 34.9931 | 34.9931 | MATCH | MATCH | Added 2026-09-24 by 148-02: the figure the allocation is charged for this parcel-month (Brent's Q2 ruling). Recomputed as the run's final_af 27.9945 divided by the deployment's groundwater_efficiency 0.800 = 34.9931, and against the stamped column (agrees to 0.0000). Captured on the rebuilt demonstration (candidate `7950c81`). Independence: different identity. |
...
| `FIG-accounting-023` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:194` | All 18 assigned use areas › Surface | `balance.supplies.surface` | `accounting/views.py:630` | `account_consumptive_balance` | `parcels_parcelledger` | 5,500.97 | 5500.97 | MATCH | MATCH | New site, 143-01 (2026-09-08): the Use area breakdown table's footer, added so the panel and the table are visibly one thing (R-039). It renders the SAME context value as `FIG-accounting-012` above, one variable and one number, so the recomputation at that row is the recomputation here. What makes the rows addable (DESIGN.md → *Tables that explain their numbers*, rule 5): one quantity, one period, every use area assigned to this account (eighteen, per MER-ACCT-001's own parcel-assignment count in this section's own screens note above), and the footer's label says so. |
| `FIG-accounting-024` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:195` | All 18 assigned use areas › Groundwater | `balance.supplies.groundwater` | `accounting/views.py:630` | `account_consumptive_balance` | `parcels_parcelledger` | 0.00 | 0.00 | MATCH | MATCH | New site, 143-01 (2026-09-08). Renders the SAME context value as `FIG-accounting-013` above; same independence and same addability note as `FIG-accounting-023`. |
| `FIG-accounting-025` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:196` | All 18 assigned use areas › Rain | `balance.supplies.precip` | `accounting/views.py:630` | `account_consumptive_balance` | `accounting_calculationrun` | 420.12 | 420.12 | MATCH | MATCH | New site, 143-01 (2026-09-08). Renders the SAME context value as `FIG-accounting-014` above; same independence and same addability note as `FIG-accounting-023`. |
| `FIG-accounting-026` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:197` | All 18 assigned use areas › Total | `balance.supply_total` | `accounting/views.py:630` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun, accounting_wateraccountparcel` | 5,921.09 | 5921.09 | MATCH | MATCH | New site, 143-01 (2026-09-08). Renders the SAME context value as `FIG-accounting-011` above; same independence and same addability note as `FIG-accounting-023`. |
| `FIG-accounting-027` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:198` | All 18 assigned use areas › Consumptive use | `balance.consumptive_use_gross` | `accounting/views.py:630` | `account_consumptive_balance` | `accounting_calculationrun` | 4,589.61 | 4589.61 | MATCH | MATCH | New site, 143-01 (2026-09-08). Renders the SAME context value as `FIG-accounting-015` above; same independence and same addability note as `FIG-accounting-023`. |
| `FIG-accounting-028` | `/accounting/accounts/12/?period=2` | `templates/accounting/partials/_account_balances.html:199` | All 18 assigned use areas › Balance | `balance.net_vs_supply` | `accounting/views.py:630` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1,331.48 | 1331.48 | MATCH | MATCH | New site, 143-01 (2026-09-08). Renders the SAME context value as `FIG-accounting-016` above; same independence and same addability note as `FIG-accounting-023`. `tests/test_account_balances.py` holds the footer's own six cells equal to the panel's, so this row and the panel row cannot drift apart unnoticed. |
| `FIG-accounting-044` | `/accounting/allocations/?period=2` | `templates/accounting/partials/_allocations_list_results.html:58` | Allocation | `alloc.allocation_acre_feet` | `accounting/views.py:599`, ordered at `:601` | `— (model field; queryset built at accounting/views.py:599)` | `accounting_allocationplan` | 750.32 | 750.32 | MATCH | MATCH | Independence: restatement. The same 750.32 the dashboard's zone table shows for this zone, on a second screen. 143-06: the Name column is gone (R-032, it repeated the zone, water type and period the next three columns already state), the zone name became the row's own link to the zone page (R-033), and the header's "(AF)" moved to the card head's unit line (rule 4). The value and its column did not move, only the site line, from :34 to :58. |
| `FIG-accounting-045` | `/accounting/allocations/?period=` | `templates/accounting/partials/_allocations_list_results.html:75` | All 3 groundwater allocations, WY 2025-2026 | `subtotal.total` (loop `subtotal in alloc.period_group_subtotals`) | `accounting/views.py:628`, grouped at `:635` | `— (queryset aggregate, GROUP BY reporting_period, water_type)` | `accounting_allocationplan, accounting_watertype, accounting_reportingperiod` | 4,621.27 | 4621.27 | MATCH | MATCH | New site, 143-06 (candidate A, the 2026-09-12 checkpoint): "All periods" now groups by water year and closes each group with its own subtotal rows inside the table body (a `tr.row-subtotal`), never a `tfoot`, so the two years never add into 304,200.00 (R-041, ISS-164). Pinned to the first subtotal row on the "All periods" screen, the WY 2025-2026 group's groundwater figure. The recomputation is `alloc_by_water_type` in `audit/figure_ledger/sql/accounting_detail.sql`, run for reporting period id 2: independently queried, Groundwater sums to 4,621.27 over 3 rows. Independence: restatement, plus the (period, water type) grouping the view computes once for both this site and `FIG-accounting-046`. |
| `FIG-accounting-046` | `/accounting/allocations/?period=2` | `templates/accounting/partials/_allocations_list_results.html:94` | All 5 surface-water allocations, WY 2025-2026 | `subtotal.total` (loop `subtotal in allocation_subtotals`) | `accounting/views.py:628`, grouped at `:635` | `— (queryset aggregate, GROUP BY reporting_period, water_type)` | `accounting_allocationplan, accounting_watertype, accounting_reportingperiod` | 148,500.00 | 148500.00 | MATCH | MATCH | Was `FIG-accounting-045` before 143-06's renumbering, the label changed from "Surface Water · 5 allocations" to "All 5 surface-water allocations, WY 2025-2026" (rules 4 and 5: the unit left the figure for the card head, and the label now states what makes the rows addable). Now the closing `tfoot` row for ONE period chosen (`?period=2`), not the "All periods" case, which prints inside the body instead (`FIG-accounting-045`). The recomputation is unchanged: `alloc_by_water_type` in `audit/figure_ledger/sql/accounting_detail.sql`, Surface Water sums to 148,500.00 over 5 rows for reporting period id 2. Independence: restatement, plus the grouping the view no longer sums across either water type or water year (ISS-156, then R-041/ISS-164). |
| `FIG-accounting-062` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:362` | Consumptive use | `row.gross_et` | `accounting/views.py:326` | `unmet_demand_by_parcel` | `accounting_calculationrun` | 363.26 | 363.26 | MATCH | MATCH | New figure, 137-02 (ISS-157, surface 2): the dashboard's district-wide list of fields with water use recorded and no supply reported. Pinned to MER-APN-011, WY 2025-2026, the same field and period `FIG-parcels-011` pins on the field's own page. Independence: restatement, the same grouping `dashboard_unmet_demand.sql` recomputes; block D there reproduces this row within the six the section lists. |
| `FIG-accounting-063` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:363` | Not met by supplies (column, under the card title 'Fields with water use recorded and no supply reported') | `row.unmet` | `accounting/views.py:326` | `unmet_demand_by_parcel` | `accounting_calculationrun` | 330.37 | 330.37 | MATCH | MATCH | Same pinned row as above: the 330.37 AF the field's own page also shows (`FIG-parcels-011`), now on the district-wide list. Independence: restatement. |
| `FIG-accounting-064` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:371` | District total | `unmet_demand_total` | `accounting/views.py:326` | `unmet_demand_by_parcel` | `accounting_calculationrun` | 2,245.69 | 2245.69 | MATCH | MATCH | Re-measured 2026-09-24 after 148-02 Task 1 (was 1,986.50): 46 fields now carry a positive shortfall in WY 2025-2026, not 6 (measured on the candidate: `MER-APN-010/011/012/013/014/019` at their old values plus 40 more, `MER-APN-076` largest at 63.64 AF down to fractions of an acre-foot). `subtract_surface_water` now subtracts only a canal delivery's consumed share (delivered × efficiency) from ET, not the whole delivered magnitude, so surface-served no-well fields that the old full-delivery subtraction covered exactly now carry a small residual, booked the same way `FIG-parcels-011` already does on one field. The pinned field, `MER-APN-011`, itself did not move (`FIG-accounting-062`, `FIG-accounting-063`); the whole increase is the other 45 rows. Independence: restatement, the same grouping `dashboard_unmet_demand.sql` recomputes for the pinned instance; the whole-column total needs the file's `_unmet` view re-run rather than hand-tracing 46 rows. Sum of the six fields carrying a non-zero shortfall in WY 2025-2026: MER-APN-010, 011, 012, 013, 014, 019. The wet year (period id 1) carries the SAME six fields at 962.67 AF; the plan's premise that the wet year would be silent for unmet demand did not reproduce (137-02-EVIDENCE.md section 3.1), and the empty state is proved instead by the fixture test written for it. Independence: restatement. |
| `FIG-accounting-065` | `/accounting/ledger/?period=2` | `templates/accounting/partials/_ledger_list_results.html:122` | Amount | `entry.amount_acre_feet` | `accounting/views.py:965` | `— (model field)` | `parcels_parcelledger` | -84.76 | -84.7556 | MATCH | MATCH | New site, 143-05 (candidate B): the `{% if entry.amount_acre_feet < 0 %}` branch, added so a negative Amount prints a true minus sign (U+2212) instead of the ASCII hyphen `floatformat` writes. The template strips that hyphen with the `cut` filter and supplies `&minus;` itself as static markup. Same context var and same model field as `FIG-accounting-066`, the sibling `{% else %}` branch for a zero-or-positive amount; exactly one of the two renders per row. Pinned to the first row on screen under the new default order (143-05, R-020: `-effective_date, parcel__parcel_number, -created_at`), a surface diversion on MER-APN-001 dated 15 September 2026. Independence: restatement. Site line moved from :120 to :124, then to :123, at 143-05's second checkpoint, when the Source and Water type columns merged into one Water column ahead of it and a template comment was shortened by one line, then to :122 at 143-11, when the display-time zero-row override was removed from the Description cell (the sentence moved into the engine); the value did not move. |
| `FIG-accounting-066` | `/accounting/ledger/?period=2` | `templates/accounting/partials/_ledger_list_results.html:122` | Amount | `entry.amount_acre_feet` | `accounting/views.py:965` | `— (model field)` | `parcels_parcelledger` | 3.63 | 3.6270 | MATCH | MATCH | Re-pinned 2026-09-12 (143-05): the default order gained a `parcel__parcel_number` tiebreak (R-020), which moved the first row on screen from a metered groundwater reading on MER-APN-065 at -8.56 to a surface diversion on MER-APN-001 at -84.76; that row now renders through the sibling negative branch, `FIG-accounting-065`, so THIS row (the `{% else %}` branch, zero or positive) is pinned instead to the first non-negative row on screen, a recharge entry on MER-APN-003 dated 1 September 2026. Was `FIG-accounting-061` before 137-02 inserted the dashboard's new unmet-demand section ahead of it, then `FIG-accounting-064` before 143-05's renumbering (the header lost its "(AF)" too: the unit now lives in the subtitle line, rule 4). Site line moved from :120 to :124, then to :123, at 143-05's second checkpoint, when the Source and Water type columns merged into one Water column ahead of it and a template comment was shortened by one line, then to :122 at 143-11, when the display-time zero-row override was removed from the Description cell (the sentence moved into the engine); the value did not move. Independence: restatement, including the view's ordering. |
| `FIG-accounting-067` | `/accounting/ledger/?period=2` | `templates/accounting/partials/_ledger_list_results.html:154` | Credits (allocation and recharge entries) | `ledger_total_credits` | `accounting/views.py:1057` | `— (queryset aggregate)` | `parcels_parcelledger` | 14,280.77 | 14280.77 | MATCH | MATCH | Was `FIG-accounting-063` until 136-01 retired the net row above it on 2026-09-06 (ISS-155 resolved; the value did not move), then `FIG-accounting-062` until 137-02's renumbering, then `FIG-accounting-065` before 143-05's renumbering (site line moved from :119 to :156 when the table's columns were reordered; the value did not move), then site line moved again from :156 to :160, then to :159, at 143-05's second checkpoint (the Source and Water type columns merged into one Water column ahead of it and a template comment was shortened by one line; the value did not move), then to :154 at 143-11, when the display-time zero-row override was removed from the Description cell (the sentence moved into the engine). Independence: restatement. The 14,280.77 is 13,819.63 AF of allocation entries plus 461.14 AF of recharge. Not one drop of it is water delivered to anybody, and the subtitle line now says so: negative amounts are water delivered or pumped, positive amounts are credits. Finding 3. |
| `FIG-accounting-068` | `/accounting/ledger/?period=2` | `templates/accounting/partials/_ledger_list_results.html:156` | Delivered and pumped | `ledger_total_water` | `accounting/views.py:1058` | `— (queryset aggregate)` | `parcels_parcelledger` | 15,785.09 | 15785.09 | MATCH | MATCH | Re-measured 2026-09-06 after 136-01 (was −15,785.09 under the label *debits*, as `FIG-accounting-064`), then `FIG-accounting-063` until 137-02's renumbering, then `FIG-accounting-066` before 143-05's renumbering (site line moved from :121 to :158; the value did not move), then site line moved again from :158 to :162, then to :161, at 143-05's second checkpoint (the Source and Water type columns merged into one Water column ahead of it and a template comment was shortened by one line; the value did not move), then to :156 at 143-11, when the display-time zero-row override was removed from the Description cell (the sentence moved into the engine): the same rows, shown as a magnitude and named for what they are. Independence: restatement. The 15,785.09 is 11,407.71 AF of canal deliveries plus 4,377.38 AF of pumping, and the dashboard calls both of those supplies, in those exact figures. Finding 3. |
| `FIG-accounting-069` | `/accounting/methodology/preview/?parcel_id=16&period=2026-03` | `templates/accounting/partials/_methodology_preview.html:30` | Billable groundwater | `final_af` | `accounting/views.py:1609` | `evaluate_chain` | `accounting_calculationplan, accounting_calculationstep, datasync_openetcache, parcels_parcel` | 27.9945 | 27.9945 | MATCH | MATCH | Independence: **different identity**. This screen computes fresh and stores nothing, so there is no row of its own to check it against; it is held instead against the persisted calculation run for the same parcel-month, which was written by a different code path on a different day. The two agree exactly. Was `FIG-accounting-064` before 137-02's renumbering. |
| `FIG-accounting-070` | `/accounting/methodology/preview/?parcel_id=16&period=2026-03` | `templates/accounting/partials/_methodology_preview.html:60` | In (AF) | `step.input_af` | `accounting/views.py:1618` | `evaluate_chain` | `accounting_calculationstep, datasync_openetcache` | 0.0000 | 0.0000 | MATCH | MATCH | Independence: different identity, as above. Pinned to step 1. Was `FIG-accounting-065` before 137-02's renumbering. |
| `FIG-accounting-071` | `/accounting/methodology/preview/?parcel_id=16&period=2026-03` | `templates/accounting/partials/_methodology_preview.html:61` | Out (AF) | `step.output_af` | `accounting/views.py:1619` | `evaluate_chain` | `accounting_calculationstep, datasync_openetcache, parcels_parcel` | 39.8491 | 39.8491 | MATCH | MATCH | Independence: **different identity**. Same raw-satellite rebuild as `FIG-accounting-004`. The live preview reproduces all five stored steps identically, which is the thing this screen exists to promise and the thing nothing else tests. Was `FIG-accounting-066` before 137-02's renumbering. |
| `FIG-accounting-072` | `/accounting/methodology/preview/?parcel_id=16&period=2026-03` | `templates/accounting/partials/_methodology_preview.html:68` | final … AF | `final_af` | `accounting/views.py:1609` | `evaluate_chain` | `accounting_calculationplan, accounting_calculationstep` | *(not rendered)* | *(unreachable)* | NO VALUE | UNVERIFIED | This is the fallback sentence shown when the saved method produces no steps at all. **All 5 steps on the active method are enabled**, so the branch cannot be reached without turning every one of them off, a configuration change this phase is not permitted to make. Was `FIG-accounting-067` before 137-02's renumbering. |
| `FIG-accounting-073` | `/accounting/reporting-periods/2/` | `templates/accounting/period_detail.html:133` | Allocations by water type (the single-type lead figure) | `subtotal.total` (`with subtotal=allocation_subtotals.0`) | `accounting/views.py:520`, grouped at `:522` | `— (queryset aggregate, GROUP BY water_type)` | `accounting_allocationplan, accounting_watertype` | *(not rendered)* | *(unreachable)* | NO VALUE | UNVERIFIED | New site, 143-06 (R-034): renders only when the period holds allocations of exactly one water type, so its figure can lead at the large `.budget-seg--result` size (rule 8) without picking a winner between two peers. Both seeded water years hold two types: WY 2025-2026 (period id 2) and WY 2024-2025 (period id 1) each carry 3 groundwater and 5 surface-water allocations, queried directly against `accounting_allocationplan`, so this branch cannot be reached without deleting every allocation of one type from a period, a data change this phase is not permitted to make. Matches the precedent at `FIG-accounting-072` (the methodology preview's own unreachable fallback). |
| `FIG-accounting-074` | `/accounting/reporting-periods/2/` | `templates/accounting/period_detail.html:148` | Allocations by water type, Groundwater (first of two) | `subtotal.total` (loop `subtotal in allocation_subtotals`) | `accounting/views.py:520`, grouped at `:522` | `— (queryset aggregate, GROUP BY water_type)` | `accounting_allocationplan, accounting_watertype` | 4,621.27 | 4621.27 | MATCH | MATCH | New site, 143-06 (R-034): the "Summary" tile row is gone; this is the period's lead panel, one peer figure per water type in the `.budget-panel` shape with no `--result` (rule 8: a period with two types has two peers, not one lead). One static site renders both figures at runtime (Groundwater then Surface Water, ordered by `water_type__name`), so this row is pinned to the first, Groundwater; the second, Surface Water at 148,500.00, is the same context value `FIG-accounting-076` below prints in the table's own footer. The recomputation is `alloc_by_water_type` in `audit/figure_ledger/sql/accounting_detail.sql` for reporting period id 2. Independence: restatement. |
| `FIG-accounting-075` | `/accounting/reporting-periods/2/` | `templates/accounting/period_detail.html:187` | Allocation | `alloc.allocation_acre_feet` | `accounting/views.py:512`, ordered at `:514` | `— (model field; queryset built at accounting/views.py:512)` | `accounting_allocationplan` | 750.32 | 750.32 | MATCH | MATCH | Was `FIG-accounting-072` before 143-06's renumbering. Re-pinned: 143-06 gave this queryset an ORDER BY (`water_type__name, zone__name`) it never had before, to match the allocations list's own order, so the first row on screen is no longer whichever row the database happened to return first. It is now Halvern Irrigation-Urban GSA, Groundwater, WY 2025-2026, the same first row `FIG-accounting-044` pins on the list page, 750.32, not the 2,809.93 (Halvern Valley GSA) the unordered queryset used to show first. The header's "(AF)" moved to the card head's unit line (rule 4); the value gained `intcomma` (no functional change at three-digit values). Independence: restatement. |
| `FIG-accounting-076` | `/accounting/reporting-periods/2/` | `templates/accounting/period_detail.html:197` | All 3 groundwater allocations, WY 2025-2026 | `subtotal.total` (loop `subtotal in allocation_subtotals`) | `accounting/views.py:520`, grouped at `:522` | `— (queryset aggregate, GROUP BY water_type)` | `accounting_allocationplan, accounting_watertype` | 4,621.27 | 4621.27 | MATCH | MATCH | New site, 143-06 (R-034): the Allocations table on the water-year page now carries the list's own footer, one row per water type, labelled with the count and the year (rule 5). Same context value as `FIG-accounting-074` above, one query, two sites. The recomputation is `alloc_by_water_type` in `audit/figure_ledger/sql/accounting_detail.sql` for reporting period id 2. Independence: restatement. |
...
`FIG-accounting-057`, `048` and `049` each carried this on 2026-09-05. The
...
The two surviving figures are `FIG-accounting-057` and `048`.
...
- **`FIG-accounting-062`** (`053` until 136-01)**.** The methodology preview's fallback line, shown when
...
|---|---|---|
| `/surface/diversion/8/` | `FIG-surface-001`, `005` to `007`, `013` | MER-POD-011-DEMO Snelling Re-Diversion. Records row pinned to **May 2026**. `013` is the linked water right's face value, `013` moved here from `_detail_pane.html` into its own partial (146-02 Task 2, D2) with no change to the value or the page it renders on. |
| `/surface/diversion/9/` | `FIG-surface-002` to `004`, `008` to `010` | MER-BPOD-001 El Nido Canal Recharge Intake. The new lead panel and the records table's own current-year subtotal, WY 2025-2026 (`accounting.services.current_period_id()`). |
| `/surface/diversion/3/` | `FIG-surface-012` | MER-POD-006-DEMO Stevinson Diversion Canal Headgate. First linked use area by parcel number: MER-APN-001, share 0.1429 (146-02 Task 2, D3 -- the inline fraction editor). |
| `/surface/rights/6/` | `FIG-surface-014` to `020`, `023` to `026` | MER-WR-010-DEMO, Halvern Hydroelectric Co. The new lead panel, its "by point of diversion" breakdown, and the records table's own current-year subtotal (Task 2 dropped the `[:12]` slice, R-121; every record now groups by water year, R-055). `014` and `015` are 146-02 Task 1's new max-rate and net-acreage figures on the identity card, both null on this pin. |
| `/surface/rights/` | `FIG-surface-011` | First row by right identifier: MER-WR-004-DEMO. |
...
so `FIG-surface-002/003/004` (the panel) and `FIG-surface-008/009/010` (the
table's own current-year subtotal row) are the identical Python values printed
twice, and likewise `FIG-surface-018` and `FIG-surface-024` on the right page.
...
`FIG-surface-021` and `FIG-surface-022` are the same two panel segments,
printed a third time in the branch of the template that renders only for a
right with a face value but no records in the current period (or, for -020,
...
than a second pinned value. `FIG-surface-026` is likewise `pod.max_rate_cfs`
...
`FIG-surface-025` whenever it renders. The other nine ids (001, 005, 006, 007,
011, 013, 016, 023, 025) are the pre-143-10 figures, renumbered first by the
143-10 insertion and again by 146-02 Task 2's (own id 012, and 013's move out
of `_detail_pane.html` into `_pod_water_right_panel.html`); six of them (001,
005, 006, 007, 011, 023) are byte-for-byte the same template expression at a
new line, and three (013, 016, 025) carry a small, stated change -- `013` and
`016` gained `|intcomma` (a thousands separator; the value did not move), and
`025` is the same `pod.max_rate_cfs`
expression now inside a conditional branch it did not used to be in (146-02
Task 2 did not touch this branch; only its id moved, from `024`). None of
...
`FIG-surface-001`, `005` to `007`, `013`. 143-10's new panel and subtotal figures
(`FIG-surface-002`/`003`/`004`/`008`/`009`/`010`, `016` to `020`, `023` to `026`)
...
| `id` | `screen` | `site` | `label` | `context_var` | `view` | `service` | `raw_tables` | `rendered` | `recomputed` | `delta` | `verdict` | `notes` |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `FIG-surface-001` | `/surface/diversion/8/` | `templates/surface/partials/_detail_pane.html:78` | Max rate (CFS) | `pod.max_rate_cfs` | `surface/views.py:230` | `—` | `surface_pointofdiversion` | 150.00 | 150.00 | MATCH | MATCH | Was line 57 before 143-10 Task 2 renumbered this template. Independence: different identity. Model field on the diversion point the view fetched. Wrapped in a conditional, so it renders only where a rate is recorded; see finding 3. |
| `FIG-surface-002` | `/surface/diversion/9/` | `templates/surface/partials/_detail_pane.html:168` | Water diverted panel, Diverted segment | `current_totals.diverted` | `surface/views.py:168-183` (`_pod_detail_context`) | `_group_diversion_records` (`surface/views.py:50-81`) | `surface_diversionrecord, accounting_reportingperiod` | 1,462.45 | 1,462.45 | MATCH | MATCH | New site, 143-10 Task 2 (R-055, the checkpoint's zone-lead ruling copied from 143-06). The current water year's (`accounting.services.current_period_id()`, WY 2025-2026) Diverted sum across this point's records; the SAME group `FIG-surface-008` reads (no second query, `_group_diversion_records` is called once and both sites read the same dict). Independent recomputation: `audit/figure_ledger/sql/surface_water_year_panels.sql`, `pod_year_sums` where `reporting_period_id = 2`. |
| `FIG-surface-003` | `/surface/diversion/9/` | `templates/surface/partials/_detail_pane.html:174` | Water diverted panel, Return flow segment | `current_totals.returned` | `surface/views.py:168-183` | `_group_diversion_records` | `surface_diversionrecord` | 0.00 | 0.00 | MATCH | MATCH | New site, 143-10 Task 2. Every one of this point's records is `to_storage` with `returned_af = 0` (it re-diverts nothing to the stream), so the segment is zero for every water year on record, not only the current one -- see cross-check in `surface_water_year_panels.sql`, `pod_year_sums`, both rows. Independence: different identity, `Sum(returned_af)`. |
| `FIG-surface-004` | `/surface/diversion/9/` | `templates/surface/partials/_detail_pane.html:180` | Water diverted panel, Retained segment (the page's one `.budget-seg--result`) | `current_totals.retained` | `surface/views.py:168-183` | `_group_diversion_records` | `surface_diversionrecord` | 1,462.45 | 1,462.45 | MATCH | MATCH | New site, 143-10 Task 2 (rule 8: the only large figure on the page). `current_totals.diverted` (`FIG-surface-002`) minus `current_totals.returned` (`FIG-surface-003`) = 1,462.45 − 0.00 = 1,462.45 by hand. Independence: restatement, verified a second way by hand subtraction, and by `surface_water_year_panels.sql`'s independently-summed `retained` column. |
| `FIG-surface-005` | `/surface/diversion/8/` | `templates/surface/partials/_diversion_records.html:59` | Diverted | `record.volume_acre_feet` | `surface/views.py:146-198` | `—` | `surface_diversionrecord` | 250.00 | 250.00 | MATCH | MATCH | Was `FIG-surface-003` at line 20. Independence: different identity. Whole column also checked, all 8 records on this diversion point, cross-check 3 in `surface_reporting.sql`. |
| `FIG-surface-006` | `/surface/diversion/8/` | `templates/surface/partials/_diversion_records.html:60` | Return flow | `record.returned_af` | `surface/views.py:146-198` | `—` | `surface_diversionrecord` | 100.00 | 100.00 | MATCH | MATCH | Was `FIG-surface-004` at line 21. Independence: different identity. This is one of only two records in the whole demonstration where this column is neither 0 nor the full diverted volume, which is why the row was pinned here. |
| `FIG-surface-007` | `/surface/diversion/8/` | `templates/surface/partials/_diversion_records.html:61` | Retained, now its own column with a `.col-sep` instead of wrapped inside a badge | `record.consumed_acre_feet` | `surface/views.py:146-198` | `DiversionRecord.consumed_acre_feet` (`surface/models.py:162`) | `surface_diversionrecord` | 150.00 | 150.00 | MATCH | MATCH | Was `FIG-surface-005` at line 23. 143-10 Task 2 moved this figure out from inside the Retained cell's type badge (R-115: no numeric cell may hold a badge) into a plain `td-num`, added the Period column's replacement (`tr.row-group` per water year), and gave the column a `.col-sep`; the value did not move. Independence: restatement of one subtraction, `abs(volume) - returned`. Strengthened by cross-check 1 in `surface_reporting.sql`, which applies the identity to all 173 records. |
| `FIG-surface-008` | `/surface/diversion/9/` | `templates/surface/partials/_diversion_records.html:95` | Records table, current water year's own closing subtotal, Diverted | `group.diverted` | `surface/views.py:168-183`, `_group_diversion_records` | `_group_diversion_records` | `surface_diversionrecord, accounting_reportingperiod` | 1,462.45 | 1,462.45 | MATCH | MATCH | New site, 143-10 Task 2 (rule 7, footer A, the checkpoint's candidate-A ruling copied from 143-06): the WY 2025-2026 group is the first `tr.row-group` (the table orders `-month`, newest year first), and this is its `tr.row-subtotal`'s Diverted cell -- the SAME dict `FIG-surface-002` reads, printed a second time. 584.98 (Feb 2026) + 877.47 (Jan 2026) = 1,462.45 by hand. Independent recomputation: `surface_water_year_panels.sql`, `pod_year_sums`. |
| `FIG-surface-009` | `/surface/diversion/9/` | `templates/surface/partials/_diversion_records.html:96` | Records table, current water year's subtotal, Return flow | `group.returned` | `surface/views.py:168-183` | `_group_diversion_records` | `surface_diversionrecord` | 0.00 | 0.00 | MATCH | MATCH | New site, 143-10 Task 2. Same dict as `FIG-surface-003`. Both of this point's WY 2025-2026 records carry `returned_af = 0.0000`. |
| `FIG-surface-010` | `/surface/diversion/9/` | `templates/surface/partials/_diversion_records.html:97` | Records table, current water year's subtotal, Retained (`.col-sep`) | `group.retained` | `surface/views.py:168-183` | `_group_diversion_records` | `surface_diversionrecord` | 1,462.45 | 1,462.45 | MATCH | MATCH | New site, 143-10 Task 2. Same dict as `FIG-surface-004`; 1,462.45 − 0.00 = 1,462.45 by hand, and the prior water year's own subtotal row (WY 2024-2025, not pinned here) independently sums to 2,924.90 in `surface_water_year_panels.sql`'s `pod_year_sums`, matching the page's second `tr.row-subtotal`. |
| `FIG-surface-011` | `/surface/rights/` | `templates/surface/partials/_list_results.html:30` | Face Value | `right.face_value_acre_feet` | `surface/views.py:355-380` | `—` | `surface_waterright` | 120000 | 120000 | MATCH | MATCH | Was `FIG-surface-006`. Independence: different identity. Shown to zero decimal places, with the unit "AF" as template text beside it rather than part of the figure. All seven rights checked whole-column, cross-check 2 in `surface_reporting.sql`. |
| `FIG-surface-012` | `/surface/diversion/3/` | `templates/surface/partials/_pod_parcel_share_value.html:6` | Linked use areas table, Share | `pp.fraction` | `surface/views.py::_pod_detail_context` | `—` | `surface_pointofdiversionparcel` | 0.1429 | 0.1429 | MATCH | UNVERIFIED | New site, 146-02 Task 2 (D3): the raw fraction, four decimals ('share 1.0000'), with an inline editor (the `wells:edit_field` GET/PATCH pattern) -- replaces the retired `share_percent` (a rounded whole percent computed in the view, never shown again after this task). MER-POD-006-DEMO Stevinson Diversion Canal Headgate has 7 linked use areas at an even 1/7 share each (0.1429); the first by parcel number is MER-APN-001. Independence: transcription -- this is a value an operator can type directly through the pencil control beside it (like `FIG-wells-008`/`009`), so there is nothing to recompute it against; the number agreeing with the stored row shows the platform redisplays what was entered, not that 0.1429 is correct for this use area's real draw. Live-checked 2026-09-21 (`django.test.Client`, force-login). |
| `FIG-surface-013` | `/surface/diversion/8/` | `templates/surface/partials/_pod_water_right_panel.html:43` | Water right panel, Face value (AF) | `water_right.face_value_acre_feet` | `surface/views.py:146-198` (`_pod_detail_context`) | `—` | `surface_waterright, surface_pointofdiversion` | 60,000.00 | 60000.00 | MATCH | MATCH | 146-02 Task 2 (D2): moved out of `_detail_pane.html` (line 307, `FIG-surface-005` before this task) into its own partial so `pod_link_right` can re-render just this card on save; the value, the stored column and the page it renders on (`/surface/diversion/8/`) did not move, only the template file and this id. Also conditional: no linked right, or a right with no face value, and nothing renders; `/surface/diversion/9/`'s OWN compliance card shows this same site's OTHER live value, 3,500.00 (right MER-WR-011-DEMO), evidence that the conditional and the lookup both work, not a second ledger row (one template line, one id). Live-checked 2026-09-21: `/surface/diversion/8/` still renders `60,000.00` in this position. |
| `FIG-surface-014` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:80` | Right information, Max rate (cfs) | `water_right.max_rate_cfs` | `surface/views.py:405-473` (`_water_right_detail_context`) | `—` | `surface_waterright` | not reachable on this pin | N/A, field is null | UNVERIFIED | UNVERIFIED | 146-02 Task 1: new field (`surface/migrations/0010_water_right_season_rate_use.py`), no seed command or migration backfill sets it, so every right in the local demonstration (including right 6) reads null and the template's `blank` filter renders "Not recorded" instead of this `floatformat` expression. Not reachable until a right is edited or imported with a value; the importer test fixture in `tests/test_water_right_door.py` puts 34.4 on A001885, proving the write path, but that right is not this page's pin. |
| `FIG-surface-015` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:98` | Right information, Net acreage | `water_right.net_acreage` | `surface/views.py:405-473` | `—` | `surface_waterright` | not reachable on this pin | N/A, field is null | UNVERIFIED | UNVERIFIED | 146-02 Task 1: new field, same reason as `FIG-surface-014` -- no seed data populates it, so right 6 reads null and shows the `blank` filter's placeholder. The importer's own test fixture puts 2,400.00 on A001885 (`tests/test_water_right_door.py::test_list_import_creates_a001885_with_season_and_rate`), proving the write path independently of this pinned page. |
| `FIG-surface-016` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:117` | Right information, Face value (AF) | `water_right.face_value_acre_feet` | `surface/views.py:405-473` (`_water_right_detail_context`) | `—` | `surface_waterright` | 60,000.00 | 60000.00 | MATCH | MATCH | Was `FIG-surface-007` at line 64. 143-10 Task 2 added the intcomma filter; the value did not move. Independence: different identity. The same stored column as `FIG-surface-011`, shown to two decimal places with a thousands separator instead of none. |
| `FIG-surface-017` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:184` | Face value against records panel, Face value segment | `water_right.face_value_acre_feet` | `surface/views.py:405-473` | `—` | `surface_waterright` | 60,000.00 | 60000.00 | MATCH | MATCH | New site, 143-10 Task 2 (R-121, the zone-lead shape copied from 143-06). Repeats `FIG-surface-016` a second time on the page, in the lead panel. Renders in this branch because right 6 both carries a face value AND has records in the current period (see `FIG-surface-021` for the branch that does not). |
| `FIG-surface-018` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:190` | Face value against records panel, Recorded segment | `current_totals.diverted` | `surface/views.py:405-473` | `_group_diversion_records` | `surface_diversionrecord, surface_pointofdiversion, accounting_reportingperiod` | 15,550.00 | 15550.00 | MATCH | MATCH | New site, 143-10 Task 2. The current water year's Diverted sum across BOTH of this right's points of diversion (R-121 drops the old `[:12]` slice, R-055: every record now groups by water year). 14,400.00 (MER-POD-010-DEMO, `FIG-surface-019`) + 1,150.00 (MER-POD-011-DEMO) = 15,550.00 by hand. Independent recomputation: `surface_water_year_panels.sql`, `right_current_year`. |
| `FIG-surface-019` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:196` | Face value against records panel, Remaining segment (the page's one `.budget-seg--result`) | `remaining` | `surface/views.py:444-447` | `_group_diversion_records` | `surface_waterright, surface_diversionrecord` | 44,450.00 | 44450.00 | MATCH | MATCH | New site, 143-10 Task 2 (rule 8). `water_right.face_value_acre_feet` (`FIG-surface-017`) minus `current_totals.diverted` (`FIG-surface-018`) = 60,000.00 − 15,550.00 = 44,450.00 by hand. Independent recomputation: `surface_water_year_panels.sql`, `right_face_value_less_current_year`. |
| `FIG-surface-020` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:209` | Face value against records panel, "By point of diversion" breakdown, first row | `row.total` | `surface/views.py:405-473` | Python `dict` grouped from `current_totals["rows"]`, sorted by name | `surface_diversionrecord, surface_pointofdiversion` | 14,400.00 | 14400.00 | MATCH | MATCH | New site, 143-10 Task 2. Sorted alphabetically, so the pinned row is MER-POD-010-DEMO Merced Falls Hydroelectric Diversion's own current-year sum (12 records at 1,200.00 each). Independent recomputation: `surface_water_year_panels.sql`, `right_current_year_by_pod`. |
| `FIG-surface-021` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:221` | Face value against records panel, Face value segment (the em-dash branch: a face value on record but no records in the current period) | `water_right.face_value_acre_feet` | `surface/views.py:405-473` | `—` | `surface_waterright` | not reachable on this pin | 60000.00 | UNVERIFIED | UNVERIFIED | New site, 143-10 Task 2. Right 6 has current-period records, so this branch does not render for it; same context variable and same stored column as `FIG-surface-017`, so the same MATCH would apply the moment a right with a face value and zero current-period records is opened. No fixture in the local demonstration currently produces this branch; Task 5's guard fixture is where this gets a value assertion (a right with a face value and no current-period records). |
| `FIG-surface-022` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:244` | "Recorded" period-panel segment (the no-face-value branch) | `current_totals.diverted` | `surface/views.py:405-473` | `_group_diversion_records` | `surface_diversionrecord` | not reachable on this pin | 15550.00 | UNVERIFIED | UNVERIFIED | New site, 143-10 Task 2. Right 6 has a face value, so this branch (the period-panel shape for a right with none) does not render for it; same context variable as `FIG-surface-018`. |
| `FIG-surface-023` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:288` | Records table, Diverted | `record.volume_acre_feet` | `surface/views.py:405-473` | `—` | `surface_diversionrecord, surface_pointofdiversion` | 1,200.00 | 1,200.00 | MATCH | MATCH | Was `FIG-surface-009` at line 174. 143-10 Task 2 dropped the `[:12]` slice and the "Recent" wording, and sorted ties by `point_of_diversion__name` (R-122); the FIRST row is still September 2026 (only MER-POD-010-DEMO has a record that month), so the pinned row and value did not move even though the table now holds 32 records instead of 12. |
| `FIG-surface-024` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:294` | Records table, current water year's own closing subtotal, Diverted | `group.diverted` | `surface/views.py:405-473` | `_group_diversion_records` | `surface_diversionrecord, surface_pointofdiversion, accounting_reportingperiod` | 15,550.00 | 15550.00 | MATCH | MATCH | New site, 143-10 Task 2 (footer A, no `tfoot`, rule 7). The SAME dict `FIG-surface-018` reads, printed a second time as the table's own closing row for the 16 WY 2025-2026 records. |
| `FIG-surface-025` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:326` | Points of diversion card, Max rate (active-curtailments branch) | `pod.max_rate_cfs` | `surface/views.py:405-473` | `—` | `surface_pointofdiversion` | 400.00 | 400.00 | MATCH | MATCH | Was `FIG-surface-008` at line 113. 143-10 Task 2 put this card beside Active curtailments in a `page-grid-2col` when curtailments exist (right 6 has one, MER-CURT-001), so this exact expression now also exists a second time in the template's no-curtailments branch (`FIG-surface-026`); the value and the pin (first point by name, MER-POD-010-DEMO) did not move. |
| `FIG-surface-026` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:375` | Points of diversion card, Max rate (no-curtailments branch, full width) | `pod.max_rate_cfs` | `surface/views.py:405-473` | `—` | `surface_pointofdiversion` | not reachable on this pin | 400.00 | UNVERIFIED | UNVERIFIED | New site, 143-10 Task 2: the SAME field and template text as `FIG-surface-025`, duplicated into the branch that renders when a right carries no active curtailment (R-121's fallback: the Points of diversion card spans full width rather than sitting alone beside nothing). Right 6 always takes the other branch; a right with no curtailment would show 400.00 identically for its own first point. |
...
| `FIG-reporting-003` | `/reporting/reports/shared-supply-check/?period=2` | `templates/reporting/shared_supply_check.html:157` | Entered | `row.your_pct` | `reporting/views.py:311` | `build_shared_supply_comparison` then `apportion_shared_supply` and `_compare_split` (`reporting/generators.py:194`) | `surface_pointofdiversionparcel, parcels_parcel` | 20.00 | 20.00 | MATCH | MATCH | 143-12 Task 4 (R-090): now prints a percent of the same weight this row always pinned (0.2000 -> 20.00%), quantized `Decimal("0.01")` `ROUND_HALF_UP` in `_compare_split`; the underlying `row.your_weight` did not move. Independence: restatement. Reproduces the four-rung ladder, including which rung fires: some stored share here differs from the untouched default, so the whole group counts as hand-set and the stored shares are used as written. Cross-check 7 adds a different identity, that each group's shares sum to exactly 1.0000, and all 8 groups do. Site line moved from :158 to :157 at 143.1-01 (2026-09-17); the value did not move. |
| `FIG-reporting-004` | `/reporting/reports/shared-supply-check/?period=2` | `templates/reporting/shared_supply_check.html:159` | ET-implied | `row.et_pct` | `reporting/views.py:311` | `build_shared_supply_comparison` then `apportion_shared_supply` and `_compare_split` (`reporting/generators.py:194`) | `accounting_calculationrun, accounting_reportingperiod, surface_pointofdiversionparcel` | 11.96 | 11.96 | MATCH | MATCH | 143-12 Task 4 (R-090): now prints a percent of the same weight this row always pinned (0.1196 -> 11.96%), quantized `Decimal("0.01")` `ROUND_HALF_UP` in `_compare_split`; the underlying `row.et_weight` did not move. Independence: restatement, including the rule that the rounding residual lands on the last use area sorted as TEXT, not as a number. That rule is visible on this very screen: MER-APN-064 shows 6.47% and MER-APN-063 shows 6.48% although MER-APN-064's demand is fractionally the higher of the two. Cross-check 6 confirms no value in this data sits on an exact rounding tie, so the two rounding rules in play cannot differ here. Site line moved from :160 to :159 at 143.1-01 (2026-09-17); the value did not move. |
| `FIG-reporting-005` | `/reporting/reports/shared-supply-check/?period=2` | `templates/reporting/shared_supply_check.html:162` | Gap points | `row.gap_points` | `reporting/views.py:311` | `build_shared_supply_comparison` (`reporting/generators.py:239`) then `_compare_split` (`reporting/generators.py:194`) | `accounting_calculationrun, surface_pointofdiversionparcel` | 8.04 | 8.04 | MATCH | MATCH | 143-12 Task 4 (R-090): now prints, in percentage points, a percent of the same weight this row always pinned (0.0804 -> 8.04 points), quantized `Decimal("0.01")` `ROUND_HALF_UP` in `_compare_split`; the underlying `row.divergence` did not move. Independence: restatement. The absolute difference of the two shares above, not rounded again. Below the 15-point flag threshold, so the pinned row carries no badge; one row in this group is flagged. Site line moved from :163 to :162 at 143.1-01 (2026-09-17); the value did not move. |
...
figures, FIG-surface-002/003/004/008/009/010/016-026, and describe the
...
pins one of them deliberately. Nobody should read `FIG-surface-007` (was
`FIG-surface-008` before 146-02 Task 2, `FIG-surface-005` before 143-10) as
evidence that this arithmetic is exercised; it is evidence that it is correct
where it is exercised, which on this data is twice. 143-10 gave POD 9,
MER-BPOD-001, its own reason to be checked twice more: every one of its
records carries a zero return flow (`FIG-surface-003`/`009`), so its new
panel and subtotal are a test of the SUM only, never of the subtraction.
...
see "143-10 Task 3: the recharge site's lead panel" below; still 26 after
143-10 Task 4 gave the well page's meter table its own water-year subtotal
(one site gained) and replaced the Irrigated parcels card's raw fraction with
a whole percent computed in the view (one site lost, since the replacement is
no longer a `floatformat` call the extractor can see; see "143-10 Task 4: the
well page's meter subtotal" below): the zone page, the monitoring stations,
the wells, the recharge basins, and the first two
...
| `/wells/10/` | Well MER-W-001, "Ag well on MER-APN-002". One screen carries all eight wells figures: the newest meter read's Totalizer and Delta and its water year's own closing Delta subtotal (137-03 and 143-10 Task 4), the newest water-level month's Close and Change (137-03, ISS-145), a monitoring record with a reference elevation, a whole-number field and a two-decimal field. The irrigated parcel's whole percent ("100% of this well's pumping") is on the same screen but carries no `FIG-wells-` id of its own -- 143-10 Task 4 computes it in the view (`round(fraction * 100)`) rather than through a template `floatformat`, which is what the extractor keys on; see "143-10 Task 4" below. |
...
ahead of them and did not otherwise change. **All eight wells ids were
renumbered again by 143-10 Task 4** (2026-09-12); see "143-10 Task 4: the well
page's meter subtotal" below for what moved and why.
...
`FIG-recharge-003` (the panel) and `FIG-recharge-009` (the table's own
...
rather than a second pinned value, the same treatment `FIG-surface-021/022/026`
...
**143-10 Task 4: the well page's meter subtotal (2026-09-12).** Task 4 copied
the diversion and recharge-event tables' own footer-A shape onto the well
page's meter-reads table: a `tr.row-subtotal` closes each water year inside
the body (rule 7, no `tfoot`), its Delta cell the `Sum` of that year's
`calculated_volume` reads, computed in `wells/measurement_history.py::meter_history`
from the rows already fetched for the reads above it -- never a second query.
The Totalizer cell of that row is left empty on purpose: a totalizer is a
running reading and is never summed, unlike the diversion table's Diverted
column or the recharge event table's Volume column, both of which the closing
row does sum. `FIG-wells-004` is the new site, recomputed independently by
`audit/figure_ledger/sql/wells_water_year_subtotals.sql`
(`bash audit/figure_ledger/run_sql.sh audit/figure_ledger/sql/wells_water_year_subtotals.sql`,
run 2026-09-12), which also states the prior water year's own subtotal
(766.72 AF) so both of the page's two `tr.row-subtotal` rows are checked, not
only the pinned newest one. Task 4 also dropped the "(AF)" and "(ft)" suffixes
from the meter and depth-to-water tables' own column headers (rule 4: the unit
now states once, in each table's own facts line above it -- "Meter
MTR-MER-W-001 &middot; reads in acre-feet (AF) &middot; the delta is the read
less the previous read" for the meter table); `FIG-wells-002/003` (Totalizer,
Delta) and `FIG-wells-005/006` (Close, Change) are the same expressions at new
lines, the value unchanged. The Irrigated parcels card's "1.00 fraction" (the
old `FIG-wells-005`) is **retired, not renumbered**: R-114 replaces the raw
fraction with a whole percent computed in the view
(`wells/views.py::_well_detail_context`, `round(fraction * 100)`, never
`widthratio` in the template), so the card now reads "100% of this well's
pumping" -- a real figure on screen, just no longer one reached through a
`floatformat` call, which is what `scripts/figure_inventory.py` keys on. It
gets no ledger row for the same reason `site.capacity_acre_feet` in the
recharge list gets one and a bare `{{ x }}` elsewhere in the platform never
would: the extractor measures `floatformat` sites, not "every number", and
this one moved off that instrument rather than off the screen. Finally, the
Monitoring data card (unchanged content) moved earlier in the template's own
walk order -- into the balance cell's `page-stack` beside Current meters and
Irrigated parcels, ahead of the Measurement history card, which used to sit
between Current meters and it -- so its `reference_elevation_ft` figure, the
old `FIG-wells-006`, becomes the new `FIG-wells-001`.
...
| `id` | `screen` | `site` | `label` | `context_var` | `view` | `service` | `raw_tables` | `rendered` | `recomputed` | `delta` | `verdict` | `notes` |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `FIG-datasync-001` | `/datasync/stations/1/` | `templates/datasync/partials/_station_detail_pane.html:135` | Location (latitude, first half of the pair) | `station.location.y` | `datasync/views.py:318` | `— (model field)` | `datasync_monitoredstation` | 37.18610 | 37.18610 | MATCH | MATCH | Re-measured 2026-09-15 (143-12, R-077): line moved 104 -> 135 when the Telemetry card gained an empty-state branch above this one; the site and its recomputation are unchanged. Independence: transcription. This site and the next share one template line, one of the two lines in the platform that carry two figures. |
| `FIG-datasync-002` | `/datasync/stations/1/` | `templates/datasync/partials/_station_detail_pane.html:135` | Location (longitude, second half of the pair) | `station.location.x` | `datasync/views.py:318` | `— (model field)` | `datasync_monitoredstation` | -120.68290 | -120.68290 | MATCH | MATCH | Re-measured 2026-09-15 (143-12, R-077): same line move as `FIG-datasync-001`, same station, same reason. Independence: transcription. |
| `FIG-datasync-003` | `/datasync/stations/1/` | `templates/datasync/partials/_station_detail_pane.html:190` | Current readings, the newest published value per sensor | `r.value` | `datasync/views.py:320`, assembled at `:296-315` | `— (selection in the view: newest published reading per measured parameter)` | `datasync_datarecordstaging` | not rendered | n/a | NO VALUE | UNVERIFIED | Re-measured 2026-09-15 (143-12, R-077): line moved 157 -> 190, and the Current Readings card that holds this site no longer renders at all on a station with no readings (the card is omitted rather than printing an empty-state sentence, so the identity card can stand alone in the row). Station 1 still carries no staged data record, so this site is unreachable on it either way; 30,217 staged records exist on other stations. |
| `FIG-datasync-004` | `/datasync/stations/1/` | `templates/datasync/partials/_station_detail_pane.html:280` | Recent records, Value | `record.value` | `datasync/views.py:319`, queryset at `:237` | `— (model field)` | `datasync_datarecordstaging` | not rendered | n/a | NO VALUE | UNVERIFIED | Re-measured 2026-09-15 (143-12, R-077): line moved 241 -> 280. Same station, same reason: the Recent records table has no staged row on station 1 to pin (and, when both it and Recent sync logs are empty, the two cards now collapse into one "Sync history" card, which station 1 also hits). |
| `FIG-geography-001` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:109` | Allocation vs. use panel, Available (the lead segment) | `b.available` | `geography/views.py:282`, computed via `zone_groundwater_budget` at `accounting/services.py:1242` | `zone_groundwater_budget` calls `available_with_carryover` | `accounting_allocationplan, accounting_allocationcarryover` | 1,053.72 | 1,053.72 | MATCH | MATCH | New site, 143-06 Task 4 (the checkpoint's zone-lead ruling, 2026-09-12: "zone-lead as mocked, including the Available segment"). The platform never rendered allocation-plus-carry-over as one figure before this panel; independent recomputation is addition on the two figures already pinned below: 750.32 (`FIG-geography-002`) + 303.40 (`FIG-geography-003`) = 1,053.72. Independence: restatement (the same `zone_groundwater_budget` call the table's Allocation and Carried forward columns read, `FIG-geography-006` and `FIG-geography-007`), verified a second way by hand addition. |
| `FIG-geography-002` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:114` | Allocation vs. use panel, Available by source › Allocation | `b.budget` | `geography/views.py:275`, assigned at `:264` | `— (arithmetic in the view)` | `accounting_allocationplan` | 750.32 | 750.32 | MATCH | MATCH | New site, 143-06 Task 4: the panel's "Available by source" breakdown repeats the table's Allocation cell (`FIG-geography-006`) at a second position on the page, the account page's own `.budget-panel` shape (143-03). Same stored column, same rounding, not re-derived a second time. Independence: transcription. |
| `FIG-geography-003` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:115` | Allocation vs. use panel, Available by source › Carried forward | `b.carryover` | `geography/views.py:276`, computed via `zone_groundwater_budget` at `accounting/services.py:1240` | `zone_groundwater_budget` calls `zone_carryover` | `accounting_allocationcarryover` | +303.40 | 303.40 | MATCH | MATCH | New site, 143-06 Task 4: repeats the table's Carried forward cell (`FIG-geography-007`) in the panel's breakdown. Independence: transcription. |
| `FIG-geography-004` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:122` | Allocation vs. use panel, Used | `b.used` | `geography/views.py:283`, computed via `zone_groundwater_budget` at `accounting/services.py:1213` | `zone_groundwater_budget` calls `zone_consumptive_balance` | `parcels_parcelledger, geography_parcelzone` | 142.14 | 142.14 | MATCH | MATCH | New site, 143-06 Task 4: repeats the table's Used cell (`FIG-geography-008`) in the panel. The "297 entries" link beside it (`geography/views.py:340-347`) is a row COUNT, not a `floatformat` site, so it carries no ledger row of its own; independently: `ParcelLedger` rows with `reporting_period_id = 2` joined to this zone's parcels through `geography_parcelzone`, counted DISTINCT, is 297 by direct query, the same filter `/accounting/ledger/?period=2&zone=2` applies. Independence: transcription (the figure) plus a direct count (the link's own number). |
| `FIG-geography-005` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:132` | Allocation vs. use panel, Remaining (the page's one `.budget-seg--result`) | `b.remaining` | `geography/views.py:285`, computed via `zone_groundwater_budget` at `accounting/services.py:1246` | `zone_groundwater_budget` calls `available_with_carryover` | `accounting_allocationplan, accounting_watertype, accounting_allocationcarryover, parcels_parcelledger, geography_parcelzone` | 911.58 | 911.58 | MATCH | MATCH | New site, 143-06 Task 4: repeats the table's Remaining cell (`FIG-geography-009`) and the dashboard's zone row (`FIG-accounting-061`) as the page's ONE lead figure, rule 8, the checkpoint's zone-lead ruling. 1,053.72 (`FIG-geography-001`) minus 142.14 (`FIG-geography-004`) is 911.58 by hand. Independence: restatement, verified a second way by hand subtraction. |
| `FIG-geography-006` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:169` | Allocation (AF) | `b.budget` | `geography/views.py:307`, assigned at `:301` | `— (arithmetic in the view)` | `accounting_allocationplan` | 750.32 | 750.32 | MATCH | MATCH | Was `FIG-geography-001` before 143-06 Task 4's renumbering (the new lead panel above the table now claims the app's first five ids). Independence: transcription. One stored allocation column, read back and rounded the same way. Agrees with the dashboard's Allocation for this district in section 1. The value did not move. |
| `FIG-geography-007` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:177` | Carried forward (AF) | `b.carryover` | `geography/views.py:276`, computed via `zone_groundwater_budget` at `accounting/services.py:1240` | `zone_groundwater_budget` calls `zone_carryover` | `accounting_allocationcarryover` | 303.40 | 303.40 | MATCH | MATCH | Was `FIG-geography-002` before 143-06 Task 4's renumbering (previously `FIG-geography-002` from 137-02, ISS-154). Pinned to Halvern Irrigation-Urban GSA, WY 2025-2026 (period id 2): the same 303.40 AF the dashboard's zone row shows for this zone and period. The value did not move. Independence: restatement. |
| `FIG-geography-008` | `/map/zones/2/` | `templates/geography/partials/_zone_detail_pane.html:179` | Used (AF), with the word "pumped" now stacked in the column header instead of a per-cell suffix | `b.used` | `geography/views.py:283`, computed via `zone_groundwater_budget` at `accounting/services.py:1213` | `zone_groundwater_budget` calls `zone_consumptive_balance` | `parcels_parcelledger, geography_parcelzone` | 142.14 | 142.14 | MATCH | MATCH | Was `FIG-geography-003` before 143-06 Task 4's renumbering (previously `FIG-geography-002` under `ISS-154`, then `FIG-geography-003` after 137-02). 143-06 Task 4 moved the word "pumped" from a per-cell suffix into the column's stacked header (`.th-stack`) because every row on this zone's table shares it; the VALUE did not move. Independence: restatement. |
...
| `FIG-recharge-003` | `/recharge/1/` | `templates/recharge/partials/_detail_pane.html:146` | Water recharged panel, Recharged segment (the page's one `.budget-seg--result`) | `current_totals.volume` | `recharge/views.py:146-235`, `_group_recharge_events` (`recharge/views.py:45-93`) | `_group_recharge_events` | `recharge_rechargeevent, accounting_reportingperiod` | 318.55 | 318.55 | MATCH | MATCH | New site, 143-10 Task 3 (R-126, the period page's sum-only shape, no operators: a recharge event has nothing to subtract). The current water year's (`accounting.services.current_period_id()`, WY 2025-2026) Volume sum across this site's events; the SAME group the event table's own closing subtotal row reads (`FIG-recharge-009`), no second query. 127.42 + 191.13 = 318.55 by hand. Independent recomputation: `recharge_water_year_panels.sql`, `site_year_sums` where `reporting_period_id = 2`. |
| `FIG-recharge-004` | `/recharge/1/` | `templates/recharge/partials/_detail_pane.html:152` | Water recharged panel, "By water type" breakdown row | `type_total` | `recharge/views.py:146-235` | Python `dict` grouped from `event_groups[...]["by_type"]` | `recharge_rechargeevent, accounting_watertype` | not reachable on this pin | 318.55 | UNVERIFIED | UNVERIFIED | New site, 143-10 Task 3. Renders only when the current period holds more than one water type (never a peer result per type, ISS-156); site 1's six events on record are all Groundwater, so this branch does not render for it. Same context variable and the same per-type sums `recharge_water_year_panels.sql`'s `site_current_year_by_type` finds independently (one type, Groundwater, 318.55 -- agreeing with `FIG-recharge-003` and explaining why the breakdown stays hidden here). |
| `FIG-recharge-005` | `/recharge/1/` | `templates/recharge/partials/_detail_pane.html:163` | Water recharged panel, "By event" breakdown row | `event.volume_acre_feet` | `recharge/views.py:146-235` | `— (model field)` | `recharge_rechargeevent` | 127.42 | 127.42 | MATCH | MATCH | New site, 143-10 (the main session's page-verdict pass): the panel lists the current period's events under its figure, the SAME rows (`event_groups[...]["rows"]`) the Event history table prints, so a lone figure no longer stands in an empty panel. The pinned row is the newest event (Feb 15-18, 2026, Groundwater); `recharge_water_year_panels.sql`'s `site_year_sums` adds these same rows to 318.55, agreeing with `FIG-recharge-003`. |
| `FIG-recharge-006` | `/recharge/1/` | `templates/recharge/partials/_detail_pane.html:171` | Water recharged panel, "Water year before" segment | `previous_totals.volume` | `recharge/views.py:146-245` | `_group_recharge_events` (the group after the current one in the periods' own order) | `recharge_rechargeevent, accounting_reportingperiod` | 637.10 | 637.10 | MATCH | MATCH | New site, 143-10 (Brent's checkpoint read, 2026-09-13: one figure alone in the panel was "a giant blank box"). The prior water year's own sum, a peer beside the current year and never added to it (ISS-155); the same figure the event table's WY 2024-2025 closing subtotal prints (`FIG-recharge-009`), and `recharge_water_year_panels.sql`'s `site_year_sums` second row. |
| `FIG-recharge-007` | `/recharge/1/` | `templates/recharge/partials/_detail_pane.html:231` | Recent measurements, Value | `m.value` | `recharge/views.py:146-235`, queryset at `:199-206` | `— (model field)` | `recharge_rechargemeasurement` | 2.59 | 2.59 | MATCH | MATCH | Was `FIG-recharge-002` at line 128. 143-10 Task 3 grouped this table by measurement type (`tr.row-group` reading "Water level, ft") in place of one flat list ordered only by date, and dropped the Type and Unit columns the group header now carries; the pinned row is the newest of six water-level readings shown, no longer the newest reading of any type (that reading, a water-quality sample at 315.26 mg/L, is still on the page, further down under its own "Water quality, mg/L" group). Independence: transcription. |
| `FIG-recharge-008` | `/recharge/1/` | `templates/recharge/partials/_event_history.html:45` | Volume | `event.volume_acre_feet` | `recharge/views.py:146-235` | `— (model field)` | `recharge_rechargeevent` | 127.42 | 127.42 | MATCH | MATCH | Was `FIG-recharge-003` at line 22. 143-10 Task 3 added the `intcomma` filter (a thousands separator; the value did not move) and grouped the table by water year (`tr.row-group` / `tr.row-subtotal`, rule 7), replacing the old flat list. Independence: transcription. |
| `FIG-recharge-009` | `/recharge/1/` | `templates/recharge/partials/_event_history.html:54` | Event table, current water year's own closing subtotal, Volume | `group.volume` | `recharge/views.py:146-235`, `_group_recharge_events` | `_group_recharge_events` | `recharge_rechargeevent, accounting_reportingperiod` | 318.55 | 318.55 | MATCH | MATCH | New site, 143-10 Task 3 (footer A, no `tfoot`, rule 7): the WY 2025-2026 group is the first `tr.row-group` (the table orders `-start_date`, newest year first), and this is its `tr.row-subtotal`'s Volume cell -- the SAME dict `FIG-recharge-003` reads, printed a second time. Independent recomputation: `recharge_water_year_panels.sql`, `site_year_sums`. |
| `FIG-recharge-010` | `/recharge/` | `templates/recharge/partials/_list_results.html:31` | Capacity per fill (AF) | `site.capacity_acre_feet` | `recharge/views.py:97-144` (`recharge_sites_list`), queryset at `:115` | `— (model field)` | `recharge_rechargesite` | 637 | 637 | MATCH | MATCH | Was `FIG-recharge-004`. 143-10 Task 3 renamed the column header from "Capacity" to "Capacity per fill (AF)" (R-125); the cell's own expression and value did not move. Independence: transcription. Rounded to whole acre-feet here and to hundredths on the basin's own page, so the same basin reads 637 in the list and 637.10 one click later. Both are right; the list is choosing not to show the tenths. |
...
| `FIG-setup-002` | `/setup/` | `templates/setup/wizard.html:69` | The district boundary dropdown: "Merced Subbasin (800.9 sq mi)" | `b.area_sq_miles` | `setup/views.py:115`, queryset at `:113` | `— (model field)` | `geography_boundary` | 800.9 | 800.9 | MATCH | MATCH | Independence: transcription, plus one genuinely independent check. The platform deliberately never computes this area, taking it from the uploaded file instead, on the reasoning that a computed figure would be OpenH2O's number rather than the district's. The database can compute it: the stored outline measures 800.949 square miles against the 800.948 the file states, a difference of about half an acre across an 800 square mile basin. |
| `FIG-wells-001` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:216` | Monitoring data, Reference elevation | `monitoring.reference_elevation_ft` | `wells/views.py:189`, fetched at `:158` | `— (model field)` | `wells_monitoringwell` | 182.4 | 182.4 | MATCH | MATCH | Was `FIG-wells-006` at line 250. 143-10 Task 4 moved this card, unchanged, into the balance cell's `page-stack` beside Current meters and Irrigated parcels (R-112); the walk order now reaches it before the Measurement history card, which used to sit between Current meters and this card and now sits below both in its own full-width row. Only three of the district's 45 wells carry a monitoring record, and only two of those three carry a reference elevation, so this card is absent from most well pages. Independence: transcription. |
| `FIG-wells-002` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:274` | Totalizer | `read.current_value` | `wells/views.py:163` | `meter_history` (`wells/measurement_history.py:60-84`) | `measurements_meterreading, wells_wellmeter` | 41,241.64 | 41241.64 | MATCH | MATCH | Was `FIG-wells-001` at line 164. 143-10 Task 4 moved the Measurement history card into its own full-width row below Identification and the three short cards (R-112); the same one-meter table, same newest read. Independence: transcription, one stored column read back and rounded the same way the template does. |
| `FIG-wells-003` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:276` | Pumped (was "Delta" until Brent asked, 2026-09-13, whether delta meant difference) | `read.calculated_volume` | `wells/views.py:163` | `meter_history` reads the stored column; no arithmetic in the view | `measurements_meterreading` | 65.91 | 65.91 | MATCH | MATCH | Was `FIG-wells-002` at line 166. 143-10 Task 4 dropped the "(AF)" suffix from this column's own header (rule 4: the unit now states once, in the meter's own facts line above the table, "Meter MTR-MER-W-001 &middot; reads in acre-feet (AF) &middot; the delta is the read less the previous read"); the cell's own expression and value did not move. Independence: different identity, recomputed two ways that could disagree with the stored column and with each other: current_value minus this row's own previous_value, and current_value minus the prior read's current_value. Both read 65.9112 AF against the stored 65.9112, on this read and, whole-column, on all 24 reads on this meter (`audit/figure_ledger/sql/well_measurement_history.sql`, cross-check X1). |
| `FIG-wells-004` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:285` | Meter table, WY 2025-2026's own closing subtotal, Pumped | `year.delta_total` | `wells/views.py:196` (`_well_detail_context`) | `meter_history`'s water-year loop (`wells/measurement_history.py:68-84`) | `measurements_meterreading` | 811.73 | 811.73 | MATCH | MATCH | New site, 143-10 Task 4 (R-111, footer A, no `tfoot`, rule 7): WY 2025-2026 is the meter table's first `tr.row-group` (the table lists reads newest first), and this is its `tr.row-subtotal`'s Delta cell -- the SUM of that water year's twelve `calculated_volume` reads, computed in Python from the same rows the reads above it already came from, never a second query. The Totalizer cell of the same row is left empty: a totalizer is a running reading and is never summed. 65.91 + 118.87 + 122.28 + 134.72 + 132.58 + 78.28 + 38.20 + 15.12 + 8.91 + 8.74 + 18.41 + 69.71 = 811.73 by hand. Independent recomputation: `audit/figure_ledger/sql/wells_water_year_subtotals.sql`, which also states the prior water year's own subtotal, 766.72, agreeing with the page's second `tr.row-subtotal`. |
| `FIG-wells-005` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:318` | Close | `row.close` | `wells/views.py:164` | `water_level_history` (`wells/measurement_history.py:103-126`) | `measurements_watermeasurement` | 101.92 | 101.92 | MATCH | MATCH | Was `FIG-wells-003` at line 195. 143-10 Task 4 dropped the "(ft)" suffix from this column's own header (rule 4: the unit now states once, in the digest's own facts line above the table) and moved this table beside the meter table in a `page-grid-2col` row instead of stacking under it; no stored column holds a month's close, the view builds it in Python as the last reading in the month, taken in America/Los_Angeles; the value did not move. Independence: restatement, the check re-derives the same selection rule rather than reading a number back. |
| `FIG-wells-006` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:319` | Change | `row.change` | `wells/views.py:164` | `water_level_history` (`wells/measurement_history.py:128-132`) | `measurements_watermeasurement` | 3.13 | 3.13 | MATCH | MATCH | Was `FIG-wells-004` at line 196. Same header change as the row above: September 2026's close (101.9163) minus August 2026's close (98.7905); the value did not move. Independence: restatement, same reasoning as FIG-wells-005. A second well, MER-W-004 (well id 13, `/wells/13/`, captured as evidence rather than a figure site), carries the two-year dry-year decline ISS-145 names: its logger's close moves from 94.15 ft in September 2025 to 109.65 ft in September 2026, a 15.50 ft rise. |
| `FIG-wells-007` | `/wells/10/` | `templates/wells/partials/_detail_pane.html:326` | Depth digest, WY 2025-2026's closing row, Net change | `year.net_change` | `wells/views.py:164` | `water_level_history` (`wells/measurement_history.py`), the sum of the year's non-null monthly changes from the rows already built | `measurements_sensormeasurement` | +14.98 | +14.98 | MATCH | MATCH | New site, 2026-09-13 (Brent's checkpoint read: the meter table closes each year with a row and the digest did not, so the two drifted a row apart from the second year on). A close is a level and never sums; the changes between closes add to the year's net change. Independent route: Sep 2026 close 101.92 less Sep 2025 close 86.94 = 14.98, the same figure by a different path (the identity holds because every month of the year has a close). Cross-check: `well_measurement_history.sql`. |
| `FIG-wells-008` | `/wells/10/` | `templates/wells/partials/_editable_field.html:32` | Year Pumping Began | `ef.value` (whole-number branch) | `wells/views.py:180` | `— (model field, read straight off the well)` | `wells_well` | 1987 | 1987 | MATCH | UNVERIFIED | Same id, line moved from 24 to 32 (143-10 Task 4 added an `extra_class` doc comment and parameter above this line; the expression did not change). Independence: transcription, and that is all it can be. This template renders whichever well field the page hands it, and every one of them is a value a person typed through the pencil control beside it. There is nothing to recompute it against. The two numbers agreeing shows the platform redisplays what was entered; it says nothing about whether 1987 is the year. This is the only whole-number field the platform has. |
| `FIG-wells-009` | `/wells/10/` | `templates/wells/partials/_editable_field.html:32` | Capacity (gpm) | `ef.value` (two-decimal branch) | `wells/views.py:180` | `— (model field, read straight off the well)` | `wells_well` | 2000.00 | 2000.00 | MATCH | UNVERIFIED | Same id, line moved from 24 to 32, same reason as the row above. Same line as the row above, the second of the platform's two double-figure lines. Six well fields take this branch: capacity, depth, casing diameter, screen top, screen bottom and tested yield. The pin is capacity, the first of them in the page's own order. Same reasoning as above: nothing derives any of the six. |
| `FIG-wells-010` | `/wells/10/` | `templates/wells/partials/_irrigated_parcel_share_value.html:6` | Irrigated parcels, Share | `wip.fraction` | `wells/views.py::_well_detail_context` | `—` | `wells_wellirrigatedparcel` | 1.0000 | 1.0000 | MATCH | UNVERIFIED | New site, 146-02 Task 2 (D3), the surface app's `FIG-surface-012` mirrored for wells: the raw fraction, four decimals ('share 1.0000'), with an inline editor (the `wells:edit_field` GET/PATCH pattern the well page already used for its own fields, now reused for a related row). Replaces the retired `pumping_percent` (a rounded whole percent computed in the view). Well 10 carries exactly one irrigated parcel, MER-APN-002, at a fixed share of 1.0000. Independence: transcription, the same reasoning as `FIG-wells-008`/`009` just above -- an operator-entered value has nothing to recompute it against. Live-checked 2026-09-21 (`django.test.Client`, force-login). |
...
and that is the finding worth acting on. **The count did not move again on
2026-09-12 when 143-10 Task 4 touched the wells figures** -- it retired one
MATCH (the raw fraction, off the extractor's instrument, not off the screen;
see "143-10 Task 4" above) and added one MATCH (the meter table's new
water-year subtotal), so the section still totals 26 sites and 22 agreements.
...
Those four figures were `FIG-wells-001` through `FIG-wells-004` when 137-03
wrote them; 143-10 Task 4 renumbered the wells section again (see "143-10
Task 4: the well page's meter subtotal" above), and they are now
`FIG-wells-002`, `003`, `005` and `006` -- Totalizer, Delta, Close and Change,
in that order. All four still MATCH.
