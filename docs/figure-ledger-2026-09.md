...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `FIG-accounting-045` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:43` | Supplies | `grand_supply_total` | `accounting/views.py:194` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun, accounting_wateraccountparcel` | 17458.35 | 17458.35 | MATCH | MATCH | Independence: different identity. Summed over active accounts only. Also computed in ONE pass over the distinct union of those parcels, a second identity that would disagree if any parcel belonged to two active accounts. It does not: none does. |
...
| `FIG-accounting-050` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:93` | Balance | `grand_net` | `accounting/views.py:248` | `— (arithmetic in the view)` | `parcels_parcelledger, accounting_calculationrun` | 1249.07 | 1249.07 | MATCH | MATCH | Independence: different identity. Supplies minus consumptive use, both above. |
| `FIG-accounting-046` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:53` | Supplies by source › Surface | `grand_supply_surface` | `accounting/views.py:195` | `account_consumptive_balance` | `parcels_parcelledger` | 11407.71 | 11407.71 | MATCH | MATCH | Independence: different identity. Magnitude of the delivery rows, which are stored as negative numbers by the platform's own convention. |
| `FIG-accounting-047` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_dashboard_content.html:54` | Supplies by source › Groundwater | `grand_supply_groundwater` | `accounting/views.py:196` | `account_consumptive_balance` | `parcels_parcelledger` | 4377.38 | 4377.38 | MATCH | MATCH | Independence: different identity. Absolute value OF THE SUM of the negative non-delivery rows, not the sum of absolute values. Transcribed as written; the two differ if a positive row ever lands in that set. |
...
| `FIG-accounting-029` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:72` | Supplies › Groundwater | `row.groundwater` | `accounting/views.py:186` | `account_consumptive_balance` | `parcels_parcelledger` | 0.00 | 0.00 | MATCH | MATCH | Independence: restatement. 0.00 is a real zero here, not a suppression: this account has 216 monthly calculation runs behind it over 18 parcels, and not one groundwater or meter row in the period. |
...
| `FIG-accounting-031` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:78` | Supplies › Total | `row.supply_total` | `accounting/views.py:188` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 5921.09 | 5921.09 | MATCH | MATCH | Independence: restatement. The three supply columns added. |
| `FIG-accounting-033` | `/accounting/dashboard/?period=2` | `templates/accounting/partials/_active_accounts_table.html:84` | Balance | `row.net_vs_supply` | `accounting/views.py:189` | `account_consumptive_balance` | `parcels_parcelledger, accounting_calculationrun` | 1331.48 | 1331.48 | MATCH | MATCH | Independence: restatement. |
...
| `FIG-drinking-002` | `/drinking/production/?year=2022` | `templates/drinking/production.html:93` | Year table footer, Acre-feet row, GW column | `t.acre_feet` | `drinking/views.py:1744`, assembled in the year table's totals loop at `:1729-1744` | `-- (model field, summed in the view)` | `drinking_systemproduction` | 280.41 | 280.41 | MATCH | MATCH | New site, 146-04 Task 2 (D7, ISS-184). The standing deployment carries no production rows as of this writing (the plan's own human-verify step expects the page's empty state for exactly that reason), so this was captured by temporarily importing Le Grand CSD's real 2022 eAR export (PWSID CA2410011, the same fixture `tests/test_production_import.py::EAR_CSV_TEXT` carries verbatim) against the deployment's one `WaterSystem`, reading the page over real HTTP, then deleting the imported rows and restoring the system's real PWSID -- the deployment was left exactly as found. Twelve GW months, 91,371,800 gallons. Independence: a different identity, not a re-read of the stored column -- `audit/figure_ledger/sql/drinking_production.sql` recomputes this total two ways, `SUM(volume_acre_feet)` (transcription) and `SUM(volume_gallons) / 325851` (the conversion constant applied once to the gallons total, `drinking/models.py:694`, rather than twelve times before summing); both read 280.41. |
| `FIG-drinking-003` | `/drinking/production/?year=2022` | `templates/drinking/production.html:100` | Page summary sentence, "(N acre-feet) across every source this year" | `grand_total_af` | `drinking/views.py:1748`, `:1763` | `-- (Sum over the `totals` list already built for the footer above)` | `drinking_systemproduction` | 280.41 | 280.41 | MATCH | MATCH | New site, 146-04 Task 2 (D7, ISS-184). Same capture as `FIG-drinking-002` above (same pin, same measurement, same cleanup); this is the grand total across every type-code column, and GW is the only non-zero column on this file, so it repeats the same 280.41. Independence: same SQL file, same two methods, both 280.41. |
