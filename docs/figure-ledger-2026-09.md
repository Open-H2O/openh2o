...
| `/surface/rights/6/` | `FIG-surface-013`, `014`, `015` to `019`, `022` to `025` | MER-WR-010-DEMO, Halvern Hydroelectric Co. The new lead panel, its "by point of diversion" breakdown, and the records table's own current-year subtotal (Task 2 dropped the `[:12]` slice, R-121; every record now groups by water year, R-055). `013` and `014` are 146-02 Task 1's new max-rate and net-acreage figures on the identity card, both null on this pin. |
...
twice, and likewise `FIG-surface-017` and `FIG-surface-023` on the right page.
...
`FIG-surface-020` and `FIG-surface-021` are the same two panel segments,
...
than a second pinned value. `FIG-surface-025` is likewise `pod.max_rate_cfs`
...
`FIG-surface-024` whenever it renders. The other nine ids (001, 005, 006, 007,
008, 012, 015, 022, 024) are the pre-143-10 figures, renumbered by the
insertion; six of them (001, 006, 007, 008, 012, 022) are byte-for-byte the
same template expression at a new line, and three (005, 015, 024) carry a
small, stated change -- `005` and `015` gained `|intcomma` (a thousands
separator; the value did not move), and `024` is the same `pod.max_rate_cfs`
...
(`FIG-surface-002`/`003`/`004`/`009`/`010`/`011`, `015` to `019`, `022` to `025`)
...
| `FIG-surface-014` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:98` | Right information, Net acreage | `water_right.net_acreage` | `surface/views.py:405-473` | `—` | `surface_waterright` | not reachable on this pin | N/A, field is null | UNVERIFIED | UNVERIFIED | 146-02 Task 1: new field, same reason as `FIG-surface-013` -- no seed data populates it, so right 6 reads null and shows the `blank` filter's placeholder. The importer's own test fixture puts 2,400.00 on A001885 (`tests/test_water_right_door.py::test_list_import_creates_a001885_with_season_and_rate`), proving the write path independently of this pinned page. |
...
| `FIG-surface-023` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:294` | Records table, current water year's own closing subtotal, Diverted | `group.diverted` | `surface/views.py:405-473` | `_group_diversion_records` | `surface_diversionrecord, surface_pointofdiversion, accounting_reportingperiod` | 15,550.00 | 15550.00 | MATCH | MATCH | New site, 143-10 Task 2 (footer A, no `tfoot`, rule 7). The SAME dict `FIG-surface-017` reads, printed a second time as the table's own closing row for the 16 WY 2025-2026 records. |
| `FIG-surface-024` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:326` | Points of diversion card, Max rate (active-curtailments branch) | `pod.max_rate_cfs` | `surface/views.py:405-473` | `—` | `surface_pointofdiversion` | 400.00 | 400.00 | MATCH | MATCH | Was `FIG-surface-008` at line 113. 143-10 Task 2 put this card beside Active curtailments in a `page-grid-2col` when curtailments exist (right 6 has one, MER-CURT-001), so this exact expression now also exists a second time in the template's no-curtailments branch (`FIG-surface-025`); the value and the pin (first point by name, MER-POD-010-DEMO) did not move. |
| `FIG-surface-025` | `/surface/rights/6/` | `templates/surface/partials/_water_right_detail_pane.html:375` | Points of diversion card, Max rate (no-curtailments branch, full width) | `pod.max_rate_cfs` | `surface/views.py:405-473` | `—` | `surface_pointofdiversion` | not reachable on this pin | 400.00 | UNVERIFIED | UNVERIFIED | New site, 143-10 Task 2: the SAME field and template text as `FIG-surface-024`, duplicated into the branch that renders when a right carries no active curtailment (R-121's fallback: the Points of diversion card spans full width rather than sitting alone beside nothing). Right 6 always takes the other branch; a right with no curtailment would show 400.00 identically for its own first point. |
...
figures, FIG-surface-002/003/004/009/010/011/015-025, and describe the
...
rather than a second pinned value, the same treatment `FIG-surface-020/021/025`
