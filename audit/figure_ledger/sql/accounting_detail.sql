...
figures AS (
    ...
    -- ── The use ledger ───────────────────────────────────────────────────────
    UNION ALL SELECT 'FIG-accounting-053', 'Use ledger: first row Amount (AF)',
    ...
    UNION ALL SELECT 'FIG-accounting-054', 'Use ledger footer: Credits',
           round((SELECT credits FROM ledger_totals), 2)
    UNION ALL SELECT 'FIG-accounting-055', 'Use ledger footer: Delivered and pumped (magnitude)',
    ...
    UNION ALL SELECT 'FIG-accounting-056', 'Preview: billable groundwater (vs the stored run)',
           round((SELECT final_af FROM run_row), 4)
    UNION ALL SELECT 'FIG-accounting-057', 'Preview: step 1 In (AF)',
           round((SELECT input_af FROM run_steps WHERE ord = 1), 4)
    UNION ALL SELECT 'FIG-accounting-058', 'Preview: step 1 Out (AF), from raw satellite rows',
           round((SELECT gross_af FROM raw_run_pin), 4)
    ...
    UNION ALL SELECT 'FIG-accounting-060',
